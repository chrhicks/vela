...
export function reportSummary(report: Report) {
  if (report.status === 'stale')
    return `Stale · 0 accepted · ${report.diagnostics.length} unaccepted diagnostics`
  ...
  return (
    <>
      <text flexShrink={0} fg={props.theme.text.subdued} wrapMode="word">
        {props.label}: {props.citation.path}:{props.citation.startLine}-{props.citation.endLine}
      </text>
      <text flexShrink={0} fg={props.theme.text.default} wrapMode="word">
        {props.citation.quote}
      </text>
      <text flexShrink={0} fg={props.theme.text.subdued} wrapMode="char">
        SHA256: {props.citation.sha256}
      </text>
    </>
  )
}
  ...
  return (
    ...
    >
      <box flexDirection="column" flexShrink={0} paddingBottom={1}>
        ...
        <text fg={stale() ? props.theme.text.feedback.warning.default : props.theme.text.subdued}>
        ...
        <Show when={stale()}>
          <text fg={props.theme.text.feedback.warning.default}>
            Source changed · all diagnostics unaccepted. Run /standards again.
          </text>
        </Show>
        <Show when={props.report.status === 'incomplete'}>
          <text fg={props.theme.text.feedback.warning.default}>
            Review incomplete · available diagnostics retained; see S check status.
          </text>
        </Show>
      </box>
      ...
      <Show
        ...
        fallback={
          <Show
            when={diagnostics().length}
            fallback={
              <box flexGrow={1} minHeight={0}>
                <text fg={props.theme.text.subdued} wrapMode="word">
                  {props.report.status === 'complete'
                    ? 'No diagnostics from this review. This does not certify the source is correct.'
                    : stale()
                      ? 'No accepted diagnostics. The saved source snapshot is stale.'
                      : 'No diagnostics available. Review incomplete; press S for check status.'}
                </text>
              </box>
            }
          >
            ...
            >
              <Show when={!detailOnly()}>
                <box
                  flexDirection="column"
                  width={wide() ? '48%' : '100%'}
                  height={wide() ? '100%' : Math.min(diagnostics().length * 2 + 1, 7)}
                  minHeight={2}
                  flexShrink={0}
                >
                  ...
                  <select
                    flexGrow={1}
                    minHeight={0}
                    focused={props.focused && focus() === 'diagnostics' && !statusView()}
                    options={diagnostics().map(value => ({
                      name: `${value.location.path}:${value.location.startLine}`,
                      description: `${value.severity} · ${value.message}`,
                    }))}
                    selectedIndex={index()}
                    showDescription={true}
                    showScrollIndicator={true}
                    backgroundColor={props.theme.background.default}
                    textColor={props.theme.text.subdued}
                    focusedBackgroundColor={props.theme.background.default}
                    focusedTextColor={props.theme.text.default}
                    selectedBackgroundColor={props.theme.background.raised.high}
                    selectedTextColor={props.theme.text.default}
                    onChange={setIndex}
                  />
                </box>
              </Show>
              ...
              <scrollbox
                ...
                contentOptions={{ paddingRight: 2 }}
                focused={props.focused && focus() === 'detail' && !statusView()}
              >
      ...
      >
        <scrollbox
          ...
          focused={props.focused && statusView()}
        >
          <text flexShrink={0} fg={props.theme.text.default}>
            <b>Check status · {props.report.status}</b>
          </text>
          <text flexShrink={0} fg={props.theme.text.subdued} wrapMode="word">
            Review coverage and execution issues, separate from source diagnostics.
          </text>
          <Show when={props.report.error}>
            <text flexShrink={0} fg={props.theme.text.feedback.warning.default} wrapMode="word">
              Review error: {props.report.error}
            </text>
          </Show>
          <For each={props.report.missingEvidence}>
            {missing => (
              <>
                <text flexShrink={0} fg={props.theme.text.default} wrapMode="word">
                  Missing context: {missing.path}
                </text>
                <text flexShrink={0} fg={props.theme.text.subdued} wrapMode="word">
                  Reason: {missing.reason}
                </text>
                <text flexShrink={0} fg={props.theme.text.default} wrapMode="word">
                  Next action: {missing.nextAction}
                </text>
              </>
            )}
          </For>
      ...
      <box flexDirection="column" paddingTop={1} flexShrink={0}>
        <text fg={props.theme.text.subdued}>
          ←→ list/detail ({statusView() ? 'status' : focus()}) · ↑↓ select/scroll
        </text>
        <text fg={props.theme.text.subdued}>
          S {statusView() ? 'diagnostics' : 'check status'} · F expand · R refresh · Esc close
        </text>
      </box>
    </box>
  )
