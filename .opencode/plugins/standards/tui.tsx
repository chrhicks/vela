...
function useReport(sessionID: () => string) {
  ...
  onCleanup(rpc.events.on('updated', event => { if (event.data.sessionID === sessionID()) void refetch() }))
...
function Panel(props: { panel: PanelInput }) {
  ...
  context.keymap.layer(() => ({ mode: 'global', enabled: props.panel.focused, commands: [
    { bind: 'r', run: () => { void refetch() } },
    { bind: 'escape', run: props.panel.close },
    { bind: 'f', run: props.panel.toggleFullscreen },
  ] }))
  return <Show when={!report.error} fallback={<box flexDirection="column" padding={1}>
...
export default Plugin.define({
  ...
  setup(context) {
    const command = context.ui.slot({ append: 'app', render: () => {
      context.keymap.layer(() => ({
        mode: 'global',
        commands: [{ id: 'vela.standards.results', title: 'Inspect standards results', group: 'Vela', palette: true,
          slash: { name: 'standards-results' }, run: () => { context.ui.panel.open('vela.standards.results') } }],
      }))
      return null
    } })
    const panel = context.ui.slot({ append: 'session.panel', render: props =>
      <Show when={props.name === 'vela.standards.results'}><Panel panel={props} /></Show> })
    const indicator = context.ui.slot({ append: 'session.composer.top', render: props => <Indicator sessionID={props.sessionID} /> })
    return () => { command(); panel(); indicator() }
  },
})
