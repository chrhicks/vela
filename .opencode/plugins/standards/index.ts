...
export default Plugin.define({
  ...
  effect: ctx =>
    Effect.gen(function* () {
      ...
      const results = yield* ctx.rpc
        .register(StandardsResults, {
          latest: ({ sessionID }) =>
            Effect.gen(function* () {
          ...
          execute: (input, context) =>
            Effect.gen(function* () {
              ...
              const result = yield* Effect.tryPromise(signal =>
                checkStandards(session.location.directory, input, signal, {
                  model: reviewerModel,
              ...
              yield* results.events
                .emit('updated', { sessionID: context.sessionID })
                .pipe(Effect.orDie)
