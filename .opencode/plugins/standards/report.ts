...
export async function readReport(artifact: string): Promise<Report> {
  ...
  if (json.schemaVersion !== 3)
    return {
      schemaVersion: 3,
      artifact,
      time: typeof json.time === 'string' ? json.time : new Date().toISOString(),
      mode: 'files',
      model: 'legacy',
      status: 'incomplete',
      diagnostics: [],
      ...
      files: [],
      error:
        'Legacy detector report. Its original JSON is retained at the artifact path; run /standards for direct-review diagnostics.',
    }
  ...
  if (
    ...
    saved.report.files.some(file => !file.skipped && !file.error) &&
  ...
  try {
    ...
    for (const source of saved.sources) {
      if ((await reader.read(source.path)).sha256 !== source.sha256)
        throw new Error(`${source.path} changed since this review`)
    }
  } catch (error) {
    return {
      ...saved.report,
      artifact,
      status: 'stale',
      error: error instanceof Error ? error.message : 'Reviewed evidence is unavailable',
    }
  }
...
export function formatReport(report: Report) {
  ...
  if (report.status === 'stale')
    lines.push('Saved diagnostics are unaccepted because the evidence changed.')
  else
    for (const diagnostic of report.diagnostics) {
      const { location } = diagnostic
      lines.push(
        `${location.path}:${location.startLine}-${location.endLine} · ${diagnostic.severity} · ${diagnostic.rule}`,
        diagnostic.message,
        `  ${diagnostic.explanation}`,
        `  Evidence: ${location.quote}`,
        ...diagnostic.related.map(
          source =>
            `  Related: ${source.path}:${source.startLine}-${source.endLine} — ${source.quote}`,
        ),
  ...
  for (const missing of report.missingEvidence)
    lines.push(
      `Missing evidence for ${missing.path}: ${missing.reason}\n  Next: ${missing.nextAction}`,
    )
  ...
  lines.push(
    `${count} diagnostics · ${evaluated} targets supplied · ${report.missingEvidence.length} unresolved code questions`,
  )
  if (!count)
    lines.push(
      report.status === 'complete'
        ? 'No diagnostics reported; this is not a correctness or merge verdict.'
        : 'Incomplete or stale review is not a clean check.',
    )
