...
const capture = z
  ...
  .refine(
    value =>
      value.active === ['exposing', 'reading', 'saving', 'stopping'].includes(value.phase) &&
...
const navigation = z
  ...
  .refine(
    value =>
      new Set(value.rigs.map(rig => rig.id)).size === value.rigs.length &&
      new Set(value.captures.map(item => item.rigId)).size === value.captures.length &&
      value.captures.every(item =>
        value.rigs.some(rig => rig.id === item.rigId && rig.name === item.rigName),
      ),
  )
