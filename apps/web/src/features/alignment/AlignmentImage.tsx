...
function ExposureTime({ frame }: { readonly frame: AlignmentImageFrame }) {
  ...
  return <>{frame.capturedAtSource === 'server-estimate' ? 'Estimated exposure start' : 'Exposure started'}{' '}
    <time dateTime={frame.capturedAt}>{new Date(frame.capturedAt).toLocaleTimeString()}</time>
  </>
}

function ImageCanvas({ frame, view }: { readonly frame: AlignmentImageFrame; readonly view: AlignmentImageView }) {
  const [imageError, setImageError] = useState(false)
  const [attempt, setAttempt] = useState(0)
  useEffect(() => {
    if (!imageError) return

    const timer = setTimeout(() => { setImageError(false); setAttempt(value => value + 1) }, 1500)
  ...
  if (view === 'native') return <>
    <div className="vela-polar-native" tabIndex={0} role="region" aria-label="Native image, scroll to inspect">
      <img key={attempt} src={frame.imageUrl} width={frame.imageWidth} height={frame.imageHeight}
        style={{ width: frame.imageWidth, height: frame.imageHeight }} alt={frame.alt} {...imageEvents} />
    </div>{warning}
  </>

  if (!frame.solution) return <><img key={attempt} src={frame.imageUrl} width={frame.imageWidth} height={frame.imageHeight} alt={frame.alt} {...imageEvents} />{warning}</>

  const m = frame.solution
  const box = alignmentViewport(m, view)
  const scale = box.width / 640

  return <div className="vela-polar-image-canvas">
    <svg viewBox={`${box.left} ${box.top} ${box.width} ${box.height}`} role="img" aria-label={frame.alt}>
      <image key={attempt} href={frame.imageUrl} width={frame.imageWidth} height={frame.imageHeight} {...imageEvents} />
      <path d={`M${box.referenceX} ${box.referenceY}L${m.targetX} ${m.targetY}`} fill="none" stroke="var(--vela-polar-reference)" strokeWidth={1.5 * scale} />
      <circle data-marker="reference" cx={box.referenceX} cy={box.referenceY} r={9 * scale} fill="none" stroke="var(--vela-polar-reference)" strokeWidth={1.5 * scale} />
    ...
    <div className="vela-polar-angular-scale" aria-label={`Approximate angular scale ${angularScaleLabel(box.barArcsec)}`}><span>≈ {angularScaleLabel(box.barArcsec)}</span><i /></div>
    {warning}
  </div>
}
...
function InspectionView({ frame, readState, now, retained, noSolution }: {
  ...
  const age = frame.capturedAt ? `${Math.max(0, Math.floor((now - Date.parse(frame.capturedAt)) / 1000))} s ago` : 'Age unavailable'

  const status = frame.solution ? retained || readState !== 'current' ? 'Last known solve' : 'Latest solve'
    : noSolution ? 'No solution · Exposure retained for inspection. No alignment result yet.' : 'No alignment result yet'
  ...
  return <div className="vela-polar-inspection">
    ...
    {box?.outsideImage && <p className="vela-polar-inspection-note">Target outside captured image · Blank area has no image data.</p>}
    {view === 'fine' && box?.markersClipped && <p className="vela-polar-inspection-note">Markers outside this fine view. Use Fit both to see the correction.</p>}
    <ImageCanvas key={frame.imageUrl} frame={frame} view={view} />
    <p className="vela-polar-inspection-meta">{descriptions[view]}{box && view !== 'native' && ' · Approximate scale from camera field'}</p>
    {frame.solution && view !== 'native' && <div className="vela-polar-inspection-legend"><span><i /> Frame reference</span><span><i /> Correction target</span></div>}
  </div>
}
