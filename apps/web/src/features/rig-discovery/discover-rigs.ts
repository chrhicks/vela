...
const discoveryResult = z.object({
  candidates: z.array(z.object({
    endpoint,
    server: z.object({ name: z.string().optional(), manufacturer: z.string().optional(), manufacturerVersion: z.string().optional(), location: z.string().optional() }).optional(),
    inspectedAt: isoDate,
    ...
    disposition: z.discriminatedUnion('state', [
      ...
      z.object({ state: z.literal('already-added'), rigId: z.string().refine(value => value.trim().length > 0) }),
    ]),
  })),
  failures: z.array(z.object({ endpoint: endpoint.optional(), reason: z.enum(['scan-failed', 'unreachable', 'invalid-response', 'protocol-error']) })),
})
