...
describe('camera tangent plane geometry', () => {
  it.each([[3, 1], [30, 10]])('keeps a %s × %s degree rotated field rectangular on its camera plane', (width, height) => {
    ...
    const plane = points.map(p => {
      ...
      return [Math.atan(x * Math.cos(angle) + y * Math.sin(angle)) / r, Math.atan(-x * Math.sin(angle) + y * Math.cos(angle)) / r]
    })
