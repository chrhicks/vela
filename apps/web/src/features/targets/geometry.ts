...
export function offsetPosition(
  ...
  const x = Math.tan(eastDegrees * rad)
  const y = Math.tan(northDegrees * rad)
  const ra = center.raDegrees * rad
  const dec = center.decDegrees * rad
  const denominator = Math.cos(dec) - y * Math.sin(dec)

  return {
    raDegrees: ((ra + Math.atan2(x, denominator)) / rad + 360) % 360,
...
export function frameCorners(
  ...
  return [[-1, 1], [1, 1], [1, -1], [-1, -1]].map(([sx, sy]) => {
    ...
    return offsetPosition(
      center,
      Math.atan(x * Math.cos(angle) - y * Math.sin(angle)) / rad,
      Math.atan(x * Math.sin(angle) + y * Math.cos(angle)) / rad,
    )
  })
}
