...
import { Button, Dialog, Panel, SkyPath } from '@vela/ui'
...
function AvailableSky({ sky, targetName, stale }: { sky: TargetSkyPath, targetName: string, stale: boolean }) {
  ...
  const selected = sky.samples[selectedIndex]!
  const nowInSpan = Date.parse(sky.observedAt) >= start && Date.parse(sky.observedAt) <= end
  const skyProps = {
    targetName,
    samples: sky.samples.map(sample => ({ ...sample, label: skyTime(sample.at) })),
  ...
  const selectedDate = new Date(selected.at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })
  const light = selected.sunAltitudeDegrees < -18 ? 'Astronomical darkness' : selected.sunAltitudeDegrees < 0 ? 'Twilight' : 'Daylight'
  const details = <>
    <p className="vela-target-sky-time">{selectedDate} · {light} · 15-minute samples</p>
