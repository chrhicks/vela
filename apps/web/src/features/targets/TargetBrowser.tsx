    ...
    <section className="vela-discovery" aria-label="Target discovery">
        ...
        <div>
          ...
          <p>{search
            ...
            : noSite
              ...
              : 'Explore the night from your observing site.'}</p>
        </div>
        <Button tone="quiet" disabled={loading} onClick={refreshFromNow}>{loading ? 'Loading…' : 'Refresh'}</Button>
      </header>
      <div className="vela-discovery__snapshot" role="status">
        <span>{view
          ? `${saved ? 'Saved suggestions' : 'Calculated'} · ${dateTime(view.calculatedAt)}`
          : error ? 'Suggestions unavailable' : 'Finding tonight’s subjects…'}</span>
        <span>{view?.site
          ...
          : view ? 'Site unavailable' : 'Reading observing site'}</span>
      </div>
      <Input
        label="Find a target"
        type="search"
        ...
        value={input}
        onFocus={() => { pageNavigation.current = null }}
        onChange={event => {
          pageNavigation.current = null
      ...
      {view?.siteUnavailableReason && (
        <Panel>
          ...
          <p>These are catalog suggestions, without a location-based ranking. Refresh after the site becomes available.</p>
        </Panel>
      )}
      <div className="vela-discovery__results" role="status" ref={resultsHeading} tabIndex={-1}>
        <strong>{search
          ...
          : noSite
            ? 'Explore the catalog'
            : view?.night?.kind === 'upcoming-night' ? 'The coming night' : 'Explore tonight'}</strong>
        <span>{pending ? 'Loading selection…' : view ? `${view.total.toLocaleString()} ${search ? 'matches' : 'subjects'}` : ''}</span>
      </div>
            ...
            <article className="vela-discovery__card" key={target.id}>
              ...
              <div className="vela-discovery__body">
                <p className="vela-discovery__catalog">
                  <span>{target.catalog}{target.sizeArcminutes !== null ? ` · ${target.sizeArcminutes}′ across` : ''}</span>
                ...
                <div className="vela-discovery__window">
                  <strong>{opportunity
                    ...
                    : noSite ? 'Observing window unavailable' : 'No useful window this night'}</strong>
                  <span>{opportunity
                    ? `${clock(opportunity.startsAt)} – ${clock(opportunity.endsAt)} · above 30°`
                    : 'Explore the target to inspect its sky path.'}</span>
                </div>
                <p className="vela-discovery__reason">{opportunity
                  ...
                  : target.sky
                    ...
                    : 'Refresh with an available observing site for tonight’s context.'}</p>
      ...
      {view?.total === 0 && (
        <Panel>
          <div className="vela-target-empty">
            <h2>{view.status === 'no-darkness' && !view.query ? 'No astronomical darkness ahead' : 'No matching subjects'}</h2>
            <p>{view.status === 'no-darkness' && !view.query
              ? 'The Sun does not reach 18° below the horizon in the calculation window. You can still search the catalog.'
              : 'Try another object type, imaging filter, or catalog search.'}</p>
            <Button onClick={() => {
              setInput('')
      ...
      {view && view.total > 0 && (
        <nav className="vela-discovery__footer" aria-label="Target pages">
          <span>{view.offset + 1}–{Math.min(view.offset + view.pageSize, view.total)} of {view.total.toLocaleString()}</span>
          <div>
            <Button
              tone="quiet"
      ...
      <details className="vela-discovery__method">
        ...
        <p>Ranked for photographic interest using object type, apparent size and familiar showpieces, balanced against remaining time above 30° during astronomical darkness (Sun below −18°). These approximate windows start at the calculation time or later. Refresh recalculates from now; the list stays steady while you browse.</p>
        <p>Search includes catalog objects even without a useful window. Rankings do not predict weather, Moon interference, local obstructions, or how a subject fits your camera. Check the sky path and framing before choosing.</p>
        {view?.night && (
          <p>Calculation window: {dateTime(view.night.startsAt)} to {dateTime(view.night.endsAt)}{view.night.kind === 'polar-night' ? ' · polar night, limited to 24 hours' : ''}. Times use this browser’s timezone.</p>
        )}
      </details>
