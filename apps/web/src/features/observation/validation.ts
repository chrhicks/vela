...
const observation = z
  .object({
    ...
    connectionPreparation: z.discriminatedUnion('state', [
      z.object({
        ...
        capabilities: z.tuple([z.literal('connect-devices')]),
      }),
      z.object({
        state: z.enum(['complete', 'in-progress', 'unavailable']),
  ...
  .refine(
    value =>
      value.rig.state !== 'offline' ||
      !['available', 'complete'].includes(value.connectionPreparation.state),
  )
...
const devices = z
  .array(device)
  .refine(value => new Set(value.map(item => item.id)).size === value.length)

const failedDevice = device.extend({
  reason: z.enum([
...
const uncertainDevice = device.extend({
  reason: z.enum([
    'cancelled',
...
const resultSchema = z
  .union([
    z.object({
      ...
      reason: z.enum(['device-state-unavailable', 'identity-conflict', 'offline']),
      view: observation,
    }),
    z.object({
      outcome: z.literal('complete'),
      command: z.literal('not-needed'),
      confirmedConnected: devices.length(0),
      view: observation,
    }),
    z.object({
      outcome: z.literal('complete'),
      command: z.literal('completed'),
      confirmedConnected: devices.min(1),
      view: observation,
    }),
    z.object({
      outcome: z.literal('failed'),
      confirmedConnected: devices.length(0),
      notAttempted: devices,
      failed: failedDevice,
      uncertain: z.never().optional(),
      stoppedAfter: z.never().optional(),
      view: observation,
    }),
    z.object({
    ...
    z.object({
      ...
      confirmedConnected: devices,
      notAttempted: devices,
      uncertain: uncertainDevice,
      failed: z.never().optional(),
      stoppedAfter: z.never().optional(),
      view: observation,
    }),
  ...
  .refine(value => {
    ...
    if (value.failed)
      return (
        !('stoppedAfter' in value) && ![...confirmedIds, ...remainingIds].includes(value.failed.id)
      )

    return (
      !('failed' in value) &&
      value.stoppedAfter !== undefined &&
      confirmedIds.includes(value.stoppedAfter.id)
    )
  })
