...
const failedDevice = device.extend({
  reason: z.enum(['connection-check-failed', 'device-not-found', 'rejected', 'remained-disconnected']),
})

const uncertainDevice = device.extend({
  reason: z.enum(['cancelled', 'verification-timeout', 'verification-unavailable', 'write-outcome-unknown']),
})
...
const resultSchema = z.union([
  ...
  if (value.failed)
    return !('stoppedAfter' in value) && ![...confirmedIds, ...remainingIds].includes(value.failed.id)
