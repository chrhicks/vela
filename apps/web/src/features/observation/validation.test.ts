...
describe('observation response validation', () => {
  ...
  it('accepts complete, failed, partial, reconciled and uncertain outcomes', () => {
    ...
    for (const result of [
      ...
      { ...base, outcome: 'failed', confirmedConnected: [], failed: { ...device(1), reason: 'rejected' } },
  ...
  it('rejects malformed and contradictory operation evidence', () => {
    ...
    expect(isConnectRigDevicesResult({ outcome: 'complete', command: 'completed', confirmedConnected: [], view: observation('complete') })).toBe(false)
  })
})
