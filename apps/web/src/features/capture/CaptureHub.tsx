...
export function CaptureHub({ rigId }: { rigId: string }) {
  ...
  if (offline) status = 'Capture updates interrupted · last known state'
  else if (view?.captureReadState === 'retrying') {
    status = 'Camera observation interrupted · Retrying reads for the same exposure'
  } else if (view?.active) {
    if (view.phase === 'stopping') status = 'Stopping capture…'
    else if (view.phase === 'saving') status = 'Saving image…'
    else if (view.phase === 'reading') status = 'Receiving image…'
    else status = 'Exposing…'
  } else if (view?.phase === 'failed') status = 'Capture stopped'
  else if (view?.phase === 'stopped') status = 'Capture stopped'
  else if (!view) status = 'Loading capture state…'
  else if (!view.enabled) status = view.unavailableReason
  else if (!image) status = loading ? 'Loading latest image…' : 'No image captured yet'
  else status = null
