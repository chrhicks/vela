...
describe('capture response validation', () => {
  ...
  it('rejects unusable images and URLs outside the exact frame resource', () => {
    for (const patch of [
      ...
      { statistics: { detectedStars: 0, medianHfrPixels: 2 } },
      { statistics: { detectedStars: 2, medianHfrPixels: null } },
      { statistics: { detectedStars: 2.5, medianHfrPixels: 2 } },
      { statistics: { detectedStars: 2, medianHfrPixels: NaN } },
      { width: 0 },
      { height: 1.5 },
      { exposureSeconds: -1 },
...
it('validates confirmed retention and exact same-origin download resources', () => {
  ...
  for (const patch of [
    { saved: false },
    { rigId: 'other' },
    { fitsUrl: 'https://example.com/file.fits' },
    { previewDownloadUrl: '/api/rigs/other/saved-images/frame-1/download-preview' },
    { savedAt: 'invalid' },
  ]) {
...
it('pins native, fit and download to one declared renderer version and rejects mixed or foreign resources', () => {
  ...
  const current = {
    ...
    previewRendering: { status: 'current', version: 'background-v1' },
  }
