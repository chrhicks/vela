...
function ObservationPage({ rigId }: { rigId: string }) {
  ...
  const back = (
    <Link className="vela-rig-page__back" to={`/rigs/${encodeURIComponent(rigId)}`}>
      ← Rig details
    </Link>
  )
  ...
  return (
    <section className="vela-rig-page vela-observe vela-observe-hub">
      {back}
      <header className="capture-page__heading">
        ...
        <Badge marker={<i />} tone={uncertain ? 'warning' : presentation.tone}>
          {uncertain ? 'Confirmation needed' : presentation.badge}
        </Badge>
      </header>
      <p className="vela-capture-intro">
        {interrupted
          ? 'Waiting for rig updates. The information shown is last known.'
          : view.connectionPreparation.state === 'complete'
            ? 'Connection preparation is complete. What would you like to do?'
            : 'Check your rig’s connections, then choose an activity.'}
      </p>
      <details
        ref={readiness}
      ...
      <Panel
        title="Targets & framing"
        description="Explore the sky, compose your image, and check where the camera lands."
      >
        <Link className="vela-button" to={`/rigs/${encodeURIComponent(rigId)}/observe/targets`}>
          Choose a target →
        </Link>
      </Panel>
