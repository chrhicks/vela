...
function AlignmentPage({ rigId }: { rigId: string }) {
  ...
  const activity = offline ? 'Connection interrupted' : view.activity === 'exposing' ? 'Exposing image' : view.activity === 'solving' ? 'Plate-solving…' : view.activity === 'moving' ? `Moving to position ${view.position}…` : view.activity === 'stopping' ? 'Stopping…' : view.phase === 'finished' ? 'Alignment ended by you' : !view.active ? 'Measurements stopped' : view.activity === 'waiting' ? 'Waiting for the next image' : view.measurement ? 'Alignment updated' : 'Preparing measurement…'
