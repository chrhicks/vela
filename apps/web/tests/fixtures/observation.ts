...
export function observation(
  state: 'available' | 'complete' | 'unavailable' | 'in-progress' = 'available',
  id = 'rig-1',
): RigObservationView {
  ...
  return {
    rig: {
      id,
      name: id === 'rig-2' ? 'Askar FRA 400' : 'Seestar S30',
      state: 'reachable',
      ...
      connections: {
        ...
        disconnected: connected ? 0 : 3,
        unavailable: 0,
      },
      capabilities: ['forget'],
      devices: (['camera', 'focuser', 'telescope'] as const).map((kind, index) => ({
        id: `${id}-${index}`,
        name: ['Main camera', 'Focuser', 'Mount'][index]!,
        configuredName: kind,
        kind,
        observedAt: now,
        ...(connected
