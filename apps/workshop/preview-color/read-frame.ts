...
export function readFrame(bytes: Buffer): PreviewFrame {
  ...
  if (!ended || cards.get('SIMPLE') !== 'T' || cards.get('BITPIX') !== '32' || cards.get('NAXIS') !== '2' || cards.has('BZERO') || cards.has('BSCALE') || cards.get('ROWORDER') !== 'TOP-DOWN') {
  ...
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width < 1 || height < 1 || count > 30_000_000 || bytes.length !== 2880 + Math.ceil(count * 4 / 2880) * 2880) {
    throw new Error('Invalid FITS dimensions or payload length')
  }

  const pattern = cards.get('BAYERPAT')?.toLowerCase()

  if (pattern !== undefined && pattern !== 'rggb' && pattern !== 'grbg' && pattern !== 'gbrg' && pattern !== 'bggr') throw new Error('Unsupported Bayer pattern')
