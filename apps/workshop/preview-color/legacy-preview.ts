...
export async function capturePreviews(width: number, height: number, pixels: ArrayLike<number>, color: ImageColor = { kind: 'mono' }) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let channel = 0; channel < channels; channel++) data[y * nativeStride + x * channels + channel + 1] = display(rgb[channel]!)
    }
  }
  ...
  for (let y = 0; y < fitHeight; y++) {
    ...
    for (let x = 0; x < fitWidth; x++) {
      ...
      for (let channel = 0; channel < channels; channel++) {
        ...
        for (let sy = y * factor; sy < yEnd; sy++) for (let sx = x * factor; sx < xEnd; sx++) sum += data[sy * nativeStride + sx * channels + channel + 1]!
