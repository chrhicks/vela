  ...
  await page.waitForFunction(
    () =>
      [...document.querySelectorAll('img')].some(
        image => image.src.includes('/saved-images/') && image.complete && image.naturalWidth > 0,
      ),
    undefined,
    { timeout: 120_000 },
  )
...
try {
  const manifest = JSON.parse(
    await readFile(new URL('../.local/preview-color/manifest.json', import.meta.url), 'utf8'),
  )
  ...
  for (const fixture of manifest.fixtures) {
    ...
    const view = await (
      await page.request.get(`${base}/api/web/rigs/${rig}/saved-images/${fixture.id}`)
    ).json()
    ...
    const approved = await readFile(
      new URL(`../.local/preview-color/${fixture.key}-neutral-native.png`, import.meta.url),
    )
    ...
    assert.equal(
      ...
      hash(
        await readFile(
          new URL(`../.local/preview-color/${fixture.key}-neutral-fit.png`, import.meta.url),
        ),
      ),
    )
    ...
    await page.screenshot({
      path: new URL(`${fixture.key}-saved-fit-desktop.png`, output).pathname,
      fullPage: true,
    })
    ...
    await page.waitForFunction(() =>
      [...document.querySelectorAll('img')].some(
        image =>
          image.src.endsWith('/background-v1/preview') &&
          image.complete &&
          image.naturalWidth === 6248,
      ),
    )
    await page.screenshot({
      path: new URL(`${fixture.key}-saved-native-desktop.png`, output).pathname,
      fullPage: true,
    })
    evidence.push({
      fixture: fixture.key,
  ...
  await loadedImage()
  await page.screenshot({
    path: new URL('unsupported-original.png', output).pathname,
    fullPage: true,
  })
  ...
  const response = await page.request.post(`${base}/api/rigs/${rig}/capture/start`, {
    data: { exposureSeconds: 180, repeat: false, saveFrames: true },
  })
  ...
  const sourceView = await (
    await page.request.get(
      `${base}/api/web/rigs/${rig}/saved-images/47d1a817-c7a6-4498-b064-ab29fb80e5ec`,
    )
  ).json()

  assert.deepEqual(capture.latestImage.statistics, sourceView.image.statistics)

  const freshNative = await (
    await page.request.get(`${base}${capture.latestImage.imageUrl}`)
  ).body()

  const cooled = await readFile(
    new URL('../.local/preview-color/cooled-neutral-native.png', import.meta.url),
  )

  assert.equal(hash(freshNative), hash(cooled))

  const freshDisplayUrl = new URL(
    capture.latestImage.fitImageUrl ?? capture.latestImage.imageUrl,
    base,
  ).href
  ...
  await page.screenshot({
    path: new URL('new-replayed-capture.png', output).pathname,
    fullPage: true,
  })

  const savedFresh = await (
    await page.request.get(`${base}/api/web/rigs/${rig}/saved-images/${capture.latestImage.id}`)
  ).json()

  assert.equal(savedFresh.image.previewRendering.status, 'current')
  assert.equal(
    hash(await (await page.request.get(`${base}${savedFresh.image.previewDownloadUrl}`)).body()),
    hash(freshNative),
  )
  ...
  await loadedImage()
  await page.screenshot({
    path: new URL('collection-desktop.png', output).pathname,
    fullPage: true,
  })
  ...
  await page.screenshot({
    path: new URL('cooled-saved-phone.png', output).pathname,
    fullPage: true,
  })
  assert.deepEqual(errors, [])
  await writeFile(
    new URL('production-results.json', output),
    JSON.stringify(
      {
        evidence,
  ...
  console.log(
    JSON.stringify(
      { checkedRetained: evidence.length, freshCaptureId: capture.latestImage.id, errors },
      null,
      2,
    ),
  )
