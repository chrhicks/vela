    ...
    <div className="inspector-sections">
      <details open>
        ...
        <div className="inspector-section">
          <RangeField label="Neutral hue" max={360} min={0} onChange={(neutralHue) => onEdit({ neutralHue })} step={1} value={theme.neutralHue} suffix="°" />
          <RangeField label="Neutral chroma" max={0.08} min={0} onChange={(neutralChroma) => onEdit({ neutralChroma })} step={0.002} value={theme.neutralChroma} />
          <RangeField label="Accent hue" max={360} min={0} onChange={(accentHue) => onEdit({ accentHue })} step={1} value={theme.accentHue} suffix="°" />
          <RangeField label="Accent chroma" max={0.3} min={0.01} onChange={(accentChroma) => onEdit({ accentChroma })} step={0.005} value={theme.accentChroma} />
      ...
      <details open>
        ...
        <div className="inspector-section">
          ...
          <RangeField label="Font size" max={18} min={11} onChange={(fontSize) => onEdit({ fontSize })} step={1} value={theme.fontSize} suffix="px" />
          <RangeField label="Font weight" max={750} min={300} onChange={(fontWeight) => onEdit({ fontWeight })} step={25} value={theme.fontWeight} />
          <RangeField label="Line height" max={1.8} min={1.1} onChange={(lineHeight) => onEdit({ lineHeight })} step={0.05} value={theme.lineHeight} />
          <RangeField label="Letter spacing" max={0.08} min={-0.04} onChange={(letterSpacing) => onEdit({ letterSpacing })} step={0.005} value={theme.letterSpacing} suffix="em" />
          <RangeField label="Spacing unit" max={8} min={2} onChange={(spacingUnit) => onEdit({ spacingUnit })} step={0.5} value={theme.spacingUnit} suffix="px" />
          <RangeField label="Corner radius" max={24} min={0} onChange={(radius) => onEdit({ radius })} step={1} value={theme.radius} suffix="px" />
          <RangeField label="Border width" max={3} min={0} onChange={(borderWidth) => onEdit({ borderWidth })} step={0.5} value={theme.borderWidth} suffix="px" />
          <RangeField label="Control height" max={52} min={28} onChange={(controlHeight) => onEdit({ controlHeight })} step={1} value={theme.controlHeight} suffix="px" />
          <RangeField label="Panel padding" max={32} min={8} onChange={(panelPadding) => onEdit({ panelPadding })} step={1} value={theme.panelPadding} suffix="px" />
        </div>
      </details>
    </div>
  )
