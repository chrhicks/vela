    ...
    <main className="gallery">
        ...
        <div>
          ...
          <small>
            {contrastWarnings.length
              ? `${contrastWarnings.length} focused warning${contrastWarnings.length === 1 ? '' : 's'}`
              : `${findings.length} focused pairs clear`}
          </small>
        </div>
        <div>
          ...
          <small>
            {sourceFindings.length
              ? `${sourceFindings.length} literal color finding${sourceFindings.length === 1 ? '' : 's'}`
              : 'No literal colors in component source'}
          </small>
        </div>
        <div>
      ...
      <section className="gallery-section gallery-section--compositions">
        <div className="gallery-section__heading">
          <div>
            <h2>Compositions</h2>
            <p>Fixed arrangements built from the real primitives—not a freeform canvas.</p>
          </div>
          ...
          {compositions.map((composition) => (
            <article className="composition-card" key={composition.id}>
              ...
              <div className="composition-pair">
                <CompositionPreview composition={composition} mode="light" theme={theme} />
                <CompositionPreview composition={composition} mode="dark" theme={theme} />
              </div>
            </article>
          ))}
        </div>
