      ...
      <header className="topbar">
        ...
        <div className="topbar__controls">
            ...
            <div className="segmented" title="Preview mode">
              {(['dark', 'light'] as const).map((mode) => (
                <button
                  ...
                  key={mode}
                  onClick={() => workshop.patchSession({ mode })}
                >
                  {mode}
                </button>
              ))}
          ...
          <label className="density-control">
            ...
            />
            <output>{Math.round(workshop.session.density * 100)}%</output>
          </label>
          {!isGallery ? (
            <>
        ...
        <>
          ...
          <aside className="library-panel">
              ...
              {componentGroups.map((group) => (
                <div key={group.id}>
                  <button
                    ...
                    onClick={() => workshop.selectSpecimen(group.id, group.specimens[0]?.id ?? '')}
                  >
                  ...
                  {workshop.session.componentId === group.id && group.specimens.length > 1
                    ? group.specimens.map((entry) => (
                        <button
                          ...
                          key={entry.id}
                          onClick={() => workshop.selectSpecimen(group.id, entry.id)}
                        >
                          {entry.name}
                        </button>
                      ))
            ...
            <div className="library-note">
              ...
              <code>components/ · stable</code>
              <code>drafts/ · experimental</code>
              <p>Both refresh directly through Vite HMR.</p>
            </div>
          </aside>
          ...
          <main className="stage">
              ...
              <div className="stage-toolbar__actions">
                <label>
                  ...
                  >
                    ...
                    <option value="form">Form / settings</option>
                    <option value="toolbar">Toolbar / action row</option>
                    <option value="card">Card / data list</option>
                  </select>
                </label>
              ...
              <div className="segmented">
                {viewportPresets.map((preset) => (
                  <button
                    ...
                    key={preset.value}
                    onClick={() => workshop.patchSession({ viewport: preset.value })}
                  >
                    {preset.label}
                  </button>
                ))}
            ...
            <footer className="stage-status">
              <span className="status-dot" />
              {workshop.status}
              <span>·</span>
              <span>Stable URL updated</span>
            </footer>
          </main>
            ...
            <section className="profile-bar">
              ...
              <div className="profile-actions">
                <button
                  ...
                  disabled={workshop.activeProfile.readonly || !unsavedCount}
                  onClick={() => void workshop.save()}
                >
                  Save
                </button>
