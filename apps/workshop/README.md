...
- A draft Capture-run sketch with optional repetition, immediate stop, completed-image count, Observe continuity and retained-image inspection. Procedural exposure fixtures vary star appearance, haze and occasional streaks only when each accelerated exposure completes; no server runs or device commands.
...
- Two comparison experiments: a pinned exposure with hold-to-compare and matching crops, and two or three subjects on a shared remaining-night timeline. Both use explicit local fixtures; see [comparison experiments](../../packages/ui/src/drafts/comparison-experiments.md) for intent and adoption boundaries.
