...
const url = (width: number, scenario = 'flip') => `/?component=panel&specimen=panel-centering&profile=vela-current&mode=dark&context=isolated&viewport=${width}&prop.scenario=${scenario}`
...
for (const width of [1040, 390]) {
  test(`one request remeasures after a flip and refines to tolerance at ${width}px`, async ({ page }) => {
...
test('repeated worsening stops automatically, while a check makes no new correction', async ({ page }) => {
...
test('Stop preserves the last measurement, and changing scenario cancels pending playback', async ({ page }) => {
  ...
  expect(await shimmer.evaluate(element => getComputedStyle(element, '::after').animationName)).toBe('vela-working')
  ...
  await advanceTo(page, 'Current frame measured')
  await expect(demo.locator('.vela-centering-offset > strong')).toHaveText('86.4′')
  await expect(demo.locator('.vela-centering-measured')).toHaveAttribute('transform', /rotate\(180 /)
  ...
  await demo.getByRole('button', { name: 'Center composition', exact: true }).click()
  await page.getByRole('combobox', { name: 'Workshop scenario', exact: true }).selectOption('converges')
...
test('WorkingIndicator is discovered as stable and renders in both gallery modes', async ({ page }) => {
  const errors: string[] = []
