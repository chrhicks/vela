...
const url = (width: number, example = 'large-error', phase = 'adjusting') => `/?component=panel&specimen=panel-polar-alignment&profile=vela-current&mode=dark&context=isolated&viewport=${width + 66}&prop.example=${example}&prop.phase=${phase}`
...
for (const width of [390, 1040]) {
  test(`fit-both contains both markers; fine zoom is deliberate at ${width}px`, async ({ page }) => {
    for (const example of ['large-error', 'near-aligned', 'outside-image']) {
      ...
      await expect(demo.getByRole('button', { name: 'Fit both', exact: true })).toHaveAttribute('aria-pressed', 'true')
      ...
      const framing = await image.evaluate(element => {
        ...
        return markers.length === 2 && markers.every(marker => {
          ...
          return x > box.x + box.width * 0.08 && x < box.x + box.width * 0.92 && y > box.y + box.height * 0.08 && y < box.y + box.height * 0.92
        })
      })
      ...
      if (example === 'outside-image') await expect(demo).toContainText('Blank area has no image data')
      ...
      if (example === 'near-aligned') await expect(demo).not.toContainText('Markers outside this fine view')
  ...
  test(`baseline no-solution enlarges the same exposure and restores focus at ${width}px`, async ({ page }) => {
    ...
    expect(await dialog.evaluate(element => element.getBoundingClientRect().width)).toBeLessThanOrEqual(await demo.evaluate(element => element.clientWidth))
    ...
    await dialog.getByRole('region').evaluate(element => { element.scrollLeft = 500 })
...
test('retrying keeps last-known status beside the image, including enlargement', async ({ page }) => {
  await page.goto(url(390, 'near-aligned', 'reconnecting'))
  const demo = page.locator('.vela-polar-demo')
  await expect(demo.locator('.vela-polar-inspection-status')).toHaveText('Last known solve · 1 min 7 s ago · Retrying; pause adjustments')
  await demo.screenshot({ path: 'test-results/alignment-390-retrying.png' })
  await demo.getByRole('button', { name: 'Enlarge image' }).click()
  await expect(demo.getByRole('dialog')).toContainText('Last known solve · 1 min 7 s ago · Retrying; pause adjustments')
})
