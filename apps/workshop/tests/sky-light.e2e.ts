...
for (const mode of ['dark', 'light']) {
  test(`light windows keep phase names and colored paths aligned in ${mode} mode`, async ({
    page,
  }) => {
    ...
    await page.goto(
      `/?component=sky-path&specimen=sky-path-light-windows&profile=vela-current&mode=${mode}&context=isolated&viewport=390`,
    )
    ...
    await page.goto(
      `/?component=sky-path&specimen=sky-path-light-windows&profile=vela-current&mode=${mode}&context=isolated&viewport=390&prop.target=andromeda&prop.selectedIndex=30`,
    )
    await expect(sky.getByRole('status')).toHaveText('Astronomical darkness')

    const strokes = await sky
    ...
    await page.goto(
      `/?component=sky-path&specimen=sky-path-light-windows&profile=vela-current&mode=${mode}&context=isolated&viewport=680&prop.selectedIndex=26`,
    )
