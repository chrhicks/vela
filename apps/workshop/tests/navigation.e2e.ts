...
for (const width of [1040, 390]) {
  test(`navigation keeps rig context and returns to the right capture at ${width}px`, async ({ page }) => {
    ...
    await expect(bar.getByRole('button', { name: /17 captured/ })).toBeVisible()
    await demo.getByLabel('Try the bar with').selectOption('reading')
    await expect(bar.locator('progress')).toHaveCount(0)
    await expect(bar.getByRole('button', { name: /17 captured.*Reading image/ })).toBeVisible()
    ...
    await expect(bar.getByRole('button', { name: /Capture running on Askar/ })).toContainText('Askar')
    await bar.getByRole('button', { name: /Capture running on Askar/ }).click()
    ...
    await expect(bar.getByRole('button', { name: /Capture updates lost/ })).toBeVisible()
    await expect(bar.locator('progress')).toHaveCount(0)
    ...
    await expect(bar.getByRole('button', { name: /Capture updates lost on Askar/ })).toBeVisible()
