...
for (const width of [1040, 390]) {
  test(`camera-read recovery retains the image and receives the same exposure at ${width}px`, async ({
    page,
  }, testInfo) => {
    ...
    expect(await image.evaluate((canvas: HTMLCanvasElement) => canvas.toDataURL())).toBe(
      previousImage,
    )
    ...
    await expect(demo.getByRole('status')).toHaveText(
      'Camera observation interrupted · 3 completed',
    )
    ...
    await expect(demo.locator('.vela-capture-progress')).toContainText(
      'Receiving the same exposure 4. It was not restarted.',
    )
    await expect(count).toHaveText('3')
    expect(await image.evaluate((canvas: HTMLCanvasElement) => canvas.toDataURL())).toBe(
      previousImage,
    ...
    expect(await image.evaluate((canvas: HTMLCanvasElement) => canvas.toDataURL())).not.toBe(
      previousImage,
    )
  ...
  test(`Stop during a read outage cancels recovery and keeps uncertain cleanup failed at ${width}px`, async ({
    page,
  }, testInfo) => {
...
test('a browser/server disconnection does not offer the camera-read recovery interaction', async ({
  page,
}) => {
  ...
  await expect(demo.getByRole('button', { name: 'Start run', exact: true })).toBeDisabled()
  await expect(
    demo.getByRole('button', { name: 'Interrupt camera reads', exact: true }),
  ).toHaveCount(0)
