...
it('persists selected identity separately from replaceable inventory', async () => {
  ...
  try {
    ...
    await catalog.add({
      ...
      inventory: rig.lastObservedInventory,
    })
    ...
    expect((await (await openFileRigCatalog(path)).get('rig'))?.imagingCamera).toEqual({
      uniqueId: 'slot',
      name: 'Main camera',
    })
    expect(await catalog.setImagingCamera('unknown', { uniqueId: 'slot', name: 'Camera' })).toBe(
      false,
    )
  } finally {
