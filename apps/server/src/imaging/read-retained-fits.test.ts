...
describe('bounded retained Vela FITS reader', () => {
  it.each([16, 32] as const)(
    'decodes BITPIX %s in acquisition ADU and preserves row/Bayer origin',
    async bitpix => {
      const samples =
        bitpix === 16 ? [0, 32767, 32768, 65535] : [-2147483648, -1, 65536, 2147483647]
  ...
  it('rejects scaling ambiguity, duplicate cards, other encodings and malformed payloads', async () => {
    ...
    await expect(
      readRetainedFits(fixture(32, [0, 1, 2, 3], ['NAXIS1  = 2'.padEnd(80)])),
    ...
    await expect(readRetainedFits(Buffer.concat([signed, Buffer.alloc(2880)]))).rejects.toThrow(
      'payload length',
    )
