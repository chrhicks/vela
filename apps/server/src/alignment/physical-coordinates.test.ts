...
const sample = (frame: CaptureFixture) =>
  physicalAlignmentSample(frame.solved, { capturedAt: frame.capturedAt, exposureSeconds: 2 }, site)

describe('physical alignment coordinate boundary', () => {
  it.each(fixture.cases)(
    'recovers an independently constructed physical pole ($altitude, $azimuth)',
    reference => {
      ...
      for (const [index, value] of samples.entries()) {
        expect(
          Math.abs(value.siderealTimeDegrees - reference.samples[index]!.sidereal!) * 3600,
        ).toBeLessThan(0.5)
        expect(
          Date.parse(value.capturedAt) - Date.parse(reference.samples[index]!.capturedAt),
        ).toBe(1000)
      }
      ...
      const target = projectPhysicalAlignmentTarget(
        wcs,
  ...
  it('rejects invalid timing/site rather than manufacturing sidereal angles', () => {
    ...
    for (const exposureSeconds of [0, -1, NaN]) {
      expect(() =>
        physicalAlignmentSample(
          frame.solved,
          { capturedAt: frame.capturedAt, exposureSeconds },
          site,
        ),
      ).toThrow(/duration/)
    }
    ...
    expect(() =>
      physicalAlignmentSample(frame.solved, { capturedAt: 'invalid', exposureSeconds: 2 }, site),
    ).toThrow(/start/)
    expect(() =>
      localSiderealDegrees(new Date(frame.capturedAt), { ...site, longitudeDegrees: NaN }),
    ).toThrow(/site/)
  })
})
...
function tangentPixel(wcs: PlateWcs, point: SkyPosition) {
  ...
  const north = [
    -Math.sin(centerDec) * Math.cos(centerRa),
    -Math.sin(centerDec) * Math.sin(centerRa),
    Math.cos(centerDec),
  ]

  const center = [
    Math.cos(centerDec) * Math.cos(centerRa),
    Math.cos(centerDec) * Math.sin(centerRa),
    Math.sin(centerDec),
  ]

  const dot = (basis: number[]) =>
    basis.reduce((sum, value, index) => sum + value * vector[index]!, 0)
