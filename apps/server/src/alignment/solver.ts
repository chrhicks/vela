    ...
    async solve(frame, hint, signal) {
      ...
      try {
        ...
        const code = await runAstap(executable, ['-f', path, '-d', catalog,
          ...
          '-spd', String(hint.decDegrees + 90), '-r', '5'], signal, timeout)
        signal.throwIfAborted()
...
function encodeFits(frame: MonoFrame): Buffer {
  const { width, height, pixels } = frame
  if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width < 1 || height < 1
    || width * height > 50_000_000 || pixels.length !== width * height
    || !Number.isFinite(Date.parse(frame.capturedAt))) throw new Error('Invalid mono image')
  const card = (key: string, value: string) => `${key.padEnd(8)}= ${value.padStart(20)}`.padEnd(80)
  const header = [card('SIMPLE', 'T'), card('BITPIX', '32'), card('NAXIS', '2'),
    card('NAXIS1', String(width)), card('NAXIS2', String(height)), 'END'.padEnd(80)].join('')
  const result = Buffer.alloc(2880 + Math.ceil(pixels.length * 4 / 2880) * 2880)
  result.fill(32, 0, 2880)
  result.write(header, 'ascii')
  for (let index = 0; index < pixels.length; index++) {
    const value = pixels[index]!
    if (!Number.isInteger(value) || value < 0 || value > 2147483647) throw new Error('Mono pixels must be nonnegative signed 32-bit integers')
    result.writeInt32BE(value, 2880 + index * 4)
  }
  return result
}
