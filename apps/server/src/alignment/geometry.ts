...
export function createAlignmentBaseline(
  ...
  const nominalDirection = unadjust(points[2]!, errors.altitudeArcsec * rad / 3600,
    errors.azimuthArcsec * rad / 3600, latitudeDegrees)

  return {
    latitudeDegrees, reference, nominalDirection,
    measurement: { ...errors, correctionTarget: coordinates(rotate(nominalDirection, pole, reference.siderealTimeDegrees * rad)) },
  }
}
...
export function measureAlignment(baseline: AlignmentBaseline, sample: AlignmentSample, tracking: boolean): AlignmentMeasurement {
  ...
  for (let iteration = 0; iteration < 12; iteration++) {
    ...
    const a = scale(difference(adjust(nominal, altitude + step, azimuth, baseline.latitudeDegrees), predicted), 1 / step)
    const b = scale(difference(adjust(nominal, altitude, azimuth + step, baseline.latitudeDegrees), predicted), 1 / step)
    const aa = dot(a, a)
    const ab = dot(a, b)
    const bb = dot(b, b)
  ...
  return { ...axisErrors(axis, baseline.latitudeDegrees),
...
function localDirection(sample: AlignmentSample): Vector {
  if (![sample.raDegrees, sample.decDegrees, sample.siderealTimeDegrees].every(Number.isFinite)
    ...
    || !Number.isFinite(Date.parse(sample.capturedAt))) throw new Error('Invalid alignment sample')
...
function axisErrors(axis: Vector, latitudeDegrees: number) {
  ...
  return { altitudeArcsec: (altitude / rad - latitudeDegrees) * 3600,
...
function coordinates(v: Vector) {
  return { raDegrees: (Math.atan2(v[1], v[0]) / rad + 360) % 360,
...
function rotate(v: Vector, axis: Vector, angle: number): Vector {
  ...
  return [v[0] * c + perpendicular[0] * s + axis[0] * along,
...
function cross(a: Vector, b: Vector): Vector { return [a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]] }
