...
describe('alignment environment settings', () => {
  it('remains unavailable without an explicit endpoint', () => {
    expect(alignmentSettings({})).toBeUndefined()
  })
  it.each([undefined, 'offline', 'physical'])('accepts configured mode %s', mode => {
    ...
    expect(alignmentSettings({ ...env, VELA_ALIGNMENT_MODE: mode })?.mode).toBe(
      mode === 'physical' ? 'physical' : undefined,
    )
  })
  it.each([
    'https://mount:11111',
    'http://mount:11111/api',
    'http://user:pass@mount:11111',
    'http://mount:11111/?x=1',
  ...
  it('enables diagnostic recording only with an explicit absolute destination', () => {
    ...
    expect(
      alignmentSettings({ ...env, VELA_ALIGNMENT_DIAGNOSTICS_PATH: '/evidence/alignment' })
        ?.diagnosticsPath,
    ).toBe('/evidence/alignment')

    for (const diagnosticsPath of ['', 'relative/path']) {
      expect(() =>
        alignmentSettings({ ...env, VELA_ALIGNMENT_DIAGNOSTICS_PATH: diagnosticsPath }),
...
describe('alignment routes', () => {
  ...
  it.each([
    ...
    [
      { ...rig, imagingCamera: { uniqueId: 'different', name: 'Other camera' } },
      'Select the configured imaging camera',
    ],
    [withoutFocalLength, 'Set the effective focal length'],
    [{ ...rig, endpoint: { host: 'different', port: 11111 } }, 'not configured'],
    [
      { ...rig, lastObservedInventory: { ...rig.lastObservedInventory, devices: [] } },
      'not configured',
    ],
  ] as const)(
    'reports unmet configuration without constructing hardware %#',
    async (record, reason) => {
  ...
  it('honestly reports unavailable and unknown rigs', async () => {
    ...
    expect((await app.inject('/api/web/rigs/rig/alignment')).json()).toMatchObject({
      enabled: false,
      active: false,
    })
  ...
  it.each(['offline', 'physical'] as const)(
    'wires explicit diagnostics into %s composition',
    async mode => {
  ...
  it('composes physical start from current camera name, focal length, and a solver factory accepting observed field height', async () => {
    ...
    solverFactory(1.75)
    expect(mocks.solver).toHaveBeenCalledWith({
      executable: '/bin/astap',
      catalogPath: '/stars',
      fieldHeightDegrees: 1.75,
    })
  ...
  it.each(['stop', 'finish'])(
    'prevents a different configured rig from issuing %s to an active controller',
    async name => {
  ...
  it('rejects unknown and malformed commands without constructing hardware', async () => {
    ...
    for (const body of [null, [], { exposureSeconds: 5 }, 'start'])
      expect((await command('start', 'rig', body)).statusCode).toBe(400)
    expect(mocks.acquisition).not.toHaveBeenCalled()
    expect(operations.owner('rig')).toBeUndefined()
  })
  it('releases ownership if physical composition fails', async () => {
    const { command, operations } = setup()
    mocks.physical.mockImplementation(() => {
