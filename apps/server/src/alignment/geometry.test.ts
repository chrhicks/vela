...
describe('polar measurement from solved directions', () => {
  ...
  it('updates one frame while tracking across sidereal wrap without accumulating old error', () => {
    ...
    for (const [altitude, azimuth] of [
      ...
      check(
        measureAlignment(reference, sample(altitude!, azimuth!, 47, 2.2), true),
        altitude!,
        azimuth!,
      )
    }
  ...
  it.each([60, 80])(
    'recovers large offsets and corrections beyond five degrees at Dec %s',
    declination => {
      for (const [altitudeDegrees, azimuthDegrees] of [
        [6, -8],
        [-10, 15],
      ]) {
        ...
        check(
          measureAlignment(
            reference,
            sample(altitude / 2, azimuth / 2, -25, 2.2, declination),
            true,
          ),
          altitude / 2,
          azimuth / 2,
  ...
  it.each([
    ...
    'recovers the positive corridor with $legDegrees-degree legs and offsets ($altitude, $azimuth)',
    ({ legDegrees, altitude, azimuth }) => {
      ...
      const sidereal = (elapsedSeconds: number) =>
        (initialSidereal + elapsedSeconds * siderealDegreesPerSecond) % 360
  ...
  it('rejects a numerical jump to the second altitude solution instead of reporting a false correction', () => {
    ...
    expect(() => measureAlignment(reference, sample(0, 0, -25, 2.2), true)).toThrow(
      /ambiguous sightline geometry/,
    )
  })
  ...
  it('rejects coincident baselines and malformed measurements instead of reporting an aligned pole', () => {
    ...
    expect(() => measureAlignment(baseline(480, -360), { ...point, raDegrees: NaN }, true)).toThrow(
      /sample/,
    )
  })
})
