...
import {
  ...
  type AlignmentDiagnosticRunInfo,
  type AlignmentFrameEvidence,
} from './diagnostics.js'
...
describe('bounded alignment diagnostic recording', () => {
  ...
  it('creates isolated unique runs without removing or overwriting a previous trial', async () => {
    ...
    const journals = await Promise.all(
      directories.map(directory => readFile(join(root, directory, diagnosticJournalName), 'utf8')),
    )
  ...
  it('rejects oversized FITS before encoding pixels, and disables on the journal cap', async () => {
    ...
    const run = await create(info)
    const directory = join(root, (await readdir(root))[0]!)
    await run!.recordFrame(
      { ...frame, width: 8192, height: 8192, pixels: new Float64Array(0) },
      evidence,
    )
    await run!.recordFrame(frame, evidence)
    expect(await readdir(directory)).toEqual([diagnosticJournalName])
    expect(onError).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({ message: expect.stringContaining('128 MiB') }),
    )
    ...
    expect(
      (await readFile(join(root, secondDirectory, diagnosticJournalName), 'utf8'))
        .trim()
        .split('\n'),
    ).toHaveLength(1)
  })
  ...
  it('reports invalid opt-in roots and unsafe run IDs without rejecting into the caller', async () => {
    ...
    expect(
      await createAlignmentDiagnostics(
        await temporaryRoot(),
        onError,
      )({ ...info, runId: '../outside' }),
    ).toBeUndefined()
