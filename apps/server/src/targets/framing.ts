...
export function mountSite(mount: FramingMount): Site {
  if (mount.latitudeDegrees === undefined || mount.longitudeDegrees === undefined)
    throw new Error('The mount has not supplied an observing location.')
...
function fixedPointing(mount: FramingMount) {
  return fromMount(
    { raDegrees: mount.rightAscensionDegrees, decDegrees: mount.declinationDegrees },
    mount.coordinateSystem,
    new Date(mount.observedAt),
    mountSite(mount),
  )
}
...
export function createFramingController(
  ...
  function checkSnapshot(): FramingCheck {
    return {
      actual: state.actual,
      pointing: checkedPointing,
      configuration: checkedConfiguration,
      pointingSide: checkedSide,
    }
  }
  ...
  async function measure(
    ...
    while (true) {
      ...
      recordFraming('framing.solved', {
        ...
        'image.captured_at_source': frame.capturedAtSource ?? 'camera',
        'image.color': JSON.stringify(frame.color ?? null),
        'framing.solved.rotation_degrees': actual.rotationDegrees,
        'framing.solved.wcs': JSON.stringify(solved.wcs),
        'framing.mount.at_exposure': JSON.stringify(landed),
      ...
      recordFraming('framing.check-validation', {
        'framing.check.id': actual.checkId,
        'framing.check.current': matches,
        'framing.mount.after_exposure': JSON.stringify(finalMount),
  ...
  async function move(
    ...
    signal.throwIfAborted()
    recordFraming('framing.move.requested', {
      'framing.correction': state.centering?.correction ?? 0,
      'framing.check.id': state.actual?.checkId ?? '',
      'framing.mount.before_move': JSON.stringify(mount),
      'framing.command.j2000': JSON.stringify(destination),
      'framing.command.driver': JSON.stringify(driverPosition),
      'framing.command.coordinate_system': mount.coordinateSystem,
      'framing.command.converted_at': convertedAt.toISOString(),
    })
  ...
  function recordMeasurement(
    actual: NonNullable<FramingView['actual']>,
    correction: number,
    previous?: FramingCentering['measurements'][number],
    ...
    return {
      correction,
      checkId: actual.checkId,
      capturedAt: actual.capturedAt,
      offsetArcminutes: actual.offsetArcminutes,
      rotationDegrees: actual.rotationDegrees,
      pointingSide: checkedSide,
      pointingSideChanged: !!previous && sideChanged(previous.pointingSide, checkedSide),
      trend,
    }
  }
  ...
  function start(
    ...
    state = {
      ...
      exposureSeconds: input.exposureSeconds,
      active: true,
      phase: input.action === 'check' ? 'settling' : 'slewing',
      error: null,
      centering: null,
    }

    const attributes = {
      'framing.run.id': randomUUID(),
      'framing.action': input.action,
      'rig.id': input.rigId ?? '',
      'http.request.id': input.requestId ?? '',
      'framing.target.id': input.targetId,
      'framing.desired.ra_degrees': input.desired.raDegrees,
      'framing.desired.dec_degrees': input.desired.decDegrees,
      'framing.exposure_seconds': input.exposureSeconds,
      'framing.configuration': input.configuration,
      'framing.tolerance_arcminutes': toleranceArcminutes,
 ...
 */
export function correctedPointing(
  ...
  const a = toVector(actual)
  const b = toVector(desired)
  const point = toVector(mount)
