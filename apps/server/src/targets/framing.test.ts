...
import {
  correctedPointing,
  createFramingController,
  type FramingHardware,
  type FramingMount,
} from './framing.js'
...
function workshop(
  ...
  const frame: MonoFrame = {
    width: 100,
    height: 80,
    pixels: new Float64Array(8000),
    capturedAt: at,
  }
  ...
  const hardware: FramingHardware = {
    ...
    tracking: vi.fn(async (enabled, signal) => {
      signal.throwIfAborted()
...
describe('framing controller', () => {
  it('holds the operation lease through exposure and solving, shows actual WCS, and only corrects on an explicit next request', async () => {
    ...
    expect(view).toMatchObject({
      ...
      active: false,
      error: null,
      actual: { raDegrees: 9, decDegrees: 20, capturedAt: at },
    })
    expect(run.release).toHaveBeenCalledTimes(1)
    expect(fake.slews).toEqual([desired])
    expect(view.actual!.offsetArcminutes).toBeCloseTo(
      angularDistance(desired, { raDegrees: 9, decDegrees: 20 }) * 60,
      10,
    )
    ...
    expect(
      angularDistance(
        fake.slews[1]!,
        correctedPointing(desired, { raDegrees: 9, decDegrees: 20 }, desired),
      ),
    ).toBeLessThan(1e-10)
    expect(fake.controller.snapshot()).toMatchObject({
      phase: 'checked',
      actual: { offsetArcminutes: 0 },
    })
    expect(fake.controller.canCenter(fake.mount, 'camera+mount+focal-length')).toBe(true)
  })
  ...
  it('keeps the solved footprint and centering history through read recovery without another capture or correction', async () => {
    ...
    expect(fake.controller.snapshot()).toMatchObject({
      ...
      captureReadState: 'current',
      active: false,
      centering: { outcome: 'centered', correction: 1 },
    })
  ...
  it.each([
    ...
    'holds the lease and history through Stop during read interruption until cleanup reports $phase',
    async ({ cleanup, phase }) => {
      const fake = workshop()
      await fake.start({ check: true }).finished
      const previous = fake.controller.snapshot().actual
  ...
  it.each(['no-solution', 'solver-error'] as const)(
    'never marks an unsuccessful solve as checked: %s',
    async failure => {
      const fake = workshop()
      ...
      await run.finished
      expect(fake.controller.snapshot()).toMatchObject({
        phase: failure === 'no-solution' ? 'needs-check' : 'failed',
        active: false,
        actual: null,
  ...
  it.each(['configuration', 'movement', 'expired', 'pointing-side'] as const)(
    'rejects a stale centering check before commanding: %s',
    async cause => {
      const fake = workshop()
  ...
  it.each([desired, { raDegrees: 7, decDegrees: 20 }, { raDegrees: 10, decDegrees: 36.2 }])(
    'keeps a current check usable across small and large offsets: %j',
    async position => {
      const fake = workshop()
  ...
  it('applies the current solved correction to the newly edited destination', async () => {
    ...
    expect(
      angularDistance(fake.slews[1]!, correctedPointing(desired, checked, edited)),
    ).toBeLessThan(1e-10)
    expect(fake.controller.snapshot().desired).toEqual(edited)
    expect(fake.controller.snapshot().centering?.measurements[0]?.offsetArcminutes).toBeCloseTo(
      angularDistance(checked, edited) * 60,
      8,
    )
  })

  it.each(['west', 'unknown'] as const)(
    'refines from each fresh solve through historical worsening and recovery; initial side=%s',
    async initialSide => {
      const fake = workshop()
      ...
      fake.solver.solve = vi.fn(async () => {
        const solved = solution({
          raDegrees: desired.raDegrees,
          decDegrees: desired.decDegrees + offsets[fake.slews.length]! / 60,
        })

        if (solved.status === 'solved' && fake.slews.length > 0)
          solved.wcs.cd = [0.01, -0.002, -0.003, -0.01]
      ...
      expect(samples.map(sample => sample.trend)).toEqual([
        'starting',
        'worsened',
        'improved',
        'improved',
  ...
  it.each([
    ...
    'bounds corrections and requires an explicit new check after $outcome ($offsets)',
    async ({ offsets, outcome, moves }) => {
      const fake = workshop()
      fake.solver.solve = async () =>
        solution({
          raDegrees: desired.raDegrees,
      ...
      expect(fake.controller.snapshot()).toMatchObject({
        phase: 'checked',
        active: false,
        centering: { outcome, correction: moves },
      })
      expect(fake.controller.checkCurrent(fake.mount, 'camera+mount+focal-length')).toBe(true)
      expect(fake.controller.canCenter(fake.mount, 'camera+mount+focal-length')).toBe(false)
      await fake.start({ check: true }).finished
  ...
  it.each(['stop', 'uncertain-slew', 'solve-failed'] as const)(
    'never schedules the next correction after %s',
    async reason => {
      const fake = workshop()
      await fake.start({ check: true }).finished
      ...
      if (reason === 'uncertain-slew')
        fake.hardware.slew = vi.fn(async () => {
          throw new Error('Slew outcome unknown')
        })
      else if (reason === 'solve-failed')
        fake.solver.solve = vi.fn<PlateSolver['solve']>(async () => ({ status: 'no-solution' }))
      else
        fake.hardware.capture = vi.fn(async ({ signal, onReadout }) => {
          onReadout?.()
          arrived.resolve()

          return new Promise<MonoFrame>((_resolve, reject) =>
      ...
      expect(fake.controller.snapshot()).toMatchObject({
        ...
        centering: {
          outcome: 'interrupted',
          correction: 1,
          measurements: [expect.objectContaining({ trend: 'starting' })],
        },
      })
  ...
  it('retains the solved footprint and rechecks after changed mount readings without another slew', async () => {
    ...
    let captures = 0
    fake.hardware.capture = vi.fn(async () =>
      ++captures === 1 ? fake.frame : nextExposure.promise,
    )
    fake.solver.solve = vi.fn(async () => {
      if (captures === 1) {
    ...
    expect(fake.controller.snapshot()).toMatchObject({
      active: true,
      actual: { raDegrees: 9, decDegrees: 20 },
    })
    expect(fake.slews).toHaveLength(1)
    expect(run.release).not.toHaveBeenCalled()
  ...
  it.each([false, true])(
    'holds the lease until cancellation cleanup settles; cleanup failure=%s',
    async failedCleanup => {
      const fake = workshop()
...
describe('geometric pointing correction', () => {
  it.each([
    [
      { raDegrees: 312.5867, decDegrees: 47.35 },
      { raDegrees: 312.7049, decDegrees: 31.1199 },
    ],
    [
      { raDegrees: 359.8, decDegrees: 0 },
      { raDegrees: 0.2, decDegrees: 0 },
    ],
    [
      { raDegrees: 10, decDegrees: 89.8 },
      { raDegrees: 220, decDegrees: 89.8 },
    ],
    [
      { raDegrees: 350, decDegrees: -89.8 },
      { raDegrees: 140, decDegrees: -89.8 },
    ],
    ...
    expect(angularDistance(rotatedCompanion, target)).toBeCloseTo(
      angularDistance(companion, actual),
      9,
    )
  })

  it('preserves zero offset and refuses an ambiguous antipodal correction', () => {
    expect(angularDistance(correctedPointing(desired, desired, desired), desired)).toBeLessThan(
      1e-10,
    )
    expect(() =>
      correctedPointing(
        desired,
        { raDegrees: 0, decDegrees: 0 },
        { raDegrees: 180, decDegrees: 0 },
      ),
    ).toThrow('opposite')
  })
})
