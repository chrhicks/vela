...
export function registerAutofocus(
  ...
  async function readiness(rigId: string): Promise<{
    ...
    if (!rig.imagingCamera)
      return unavailable('Choose an imaging camera on Observe before autofocus.')
    ...
    if (detail.state === 'conflict')
      return unavailable('Rig identity needs attention before autofocus.')
    ...
    const camera = detail.inspections.find(
      device => device.providerDeviceId === rig.imagingCamera?.uniqueId && device.kind === 'camera',
    )
    ...
    if (focusers.length !== 1)
      return unavailable('Autofocus needs exactly one focuser on this Rig.', cameraView)
    ...
    if (camera.connection !== 'connected')
      return unavailable('Connect the camera before autofocus.', names)

    if (focuser.connection !== 'connected')
      return unavailable('Connect the focuser before autofocus.', names)
    const owner = operations.owner(rigId)

    if (owner && owner !== 'autofocus')
      return unavailable('Another Rig operation is in progress.', names)

    const snapshot = current()
    const focuserTelemetry = focuser.telemetry.values

    const idlePosition =
      !snapshot.active && focuserTelemetry?.kind === 'focuser'
  ...
  app.post<{ Params: { rigId: string } }>(
    '/api/rigs/:rigId/autofocus/start',
    async (request, reply) => {
      const parsed = z
        .strictObject({
      ...
      if (!request.headers['content-type']?.startsWith('application/json') || !parsed.success) {
        return reply.code(400).send({
          error:
            'Expected optional integer stepSize, offsetSteps, and exposureSeconds between 0.1 and 30.',
        })
      }
      ...
      try {
        ...
        if (parsed.data.exposureSeconds !== undefined)
        ...
        return reply
          .code(409)
          .send({ error: error instanceof Error ? error.message : 'Could not start autofocus' })
      } finally {
        if (!started) release()
      }
    },
