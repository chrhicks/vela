...
export function registerSavedImages(app: FastifyInstance, catalog: RigCatalog, store: SavedImageStore) {
  app.get<{ Params: { rigId: string } }>('/api/web/rigs/:rigId/saved-images', async (request, reply) => {
    ...
    try {
      ...
      return reply.code(503).send({ error: 'Saved images are unavailable. Check storage and try again.' })
    }
  })

  app.get<{ Params: { rigId: string, imageId: string } }>('/api/web/rigs/:rigId/saved-images/:imageId', async (request, reply) => {
    const { rigId, imageId } = request.params
    const rig = await catalog.get(rigId)
    if (!rig) return reply.code(404).send({ error: 'Rig not found' })
    try {
      const image = await store.get(rigId, imageId)
      ...
      return reply.code(503).send({ error: 'Saved image is unavailable. Check storage and try again.' })
    }
  })
  ...
  for (const resource of resources) {
    app.get<{ Params: { rigId: string, imageId: string } }>(`/api/rigs/:rigId/saved-images/:imageId/${resource.suffix}`, async (request, reply) => {
      const { rigId, imageId } = request.params
      ...
      try {
        ...
        const file = await store.file(rigId, imageId, resource.kind)
        ...
        return reply.type(resource.type).header('cache-control', 'private, max-age=3600, immutable').send(file)
      } catch (error) {
        request.log.error(error, 'Could not read saved image file')
        return reply.code(503).send({ error: 'Saved file is unavailable. Check storage and try again.' })
      }
    })
  }
}
