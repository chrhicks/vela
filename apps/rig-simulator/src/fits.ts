...
export function writeFits(width: number, height: number, pixels: Uint16Array): Buffer {
  ...
  if (pixels.some(value => value > 32767)) throw new Error('FITS pixels must fit positive signed 16-bit values')
  const card = (keyword: string, value: string) => `${keyword.padEnd(8)}= ${value.padStart(20)}`.padEnd(80)
  const header = [card('SIMPLE', 'T'), card('BITPIX', '16'), card('NAXIS', '2'), card('NAXIS1', String(width)), card('NAXIS2', String(height)), 'END'.padEnd(80)].join('')
  ...
  for (let index = 0; index < pixels.length; index++) result.writeInt16BE(pixels[index]!, headerBytes + index * 2)
