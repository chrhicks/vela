...
it('serializes the same pixels as Int32-source UInt16 ImageBytes and JSON in X/Y order', async () => {
  ...
  expect(Array.from({ length: 11 }, (_, index) => bytes.readUInt32LE(index * 4))).toEqual([
    1, 0, 4294967295, 7, 44, 2, 8, 2, 3, 2, 0,
  ])
  expect(Array.from({ length: 6 }, (_, index) => bytes.readUInt16LE(44 + index * 2))).toEqual([
    0, 10, 1, 11, 65535, 60000,
  ])
  let json = ''
...
it('requires explicit nonzero ImageBytes acceptance', () => {
  ...
  for (const value of [
    ...
    '*/*',
    'application/json',
    'application/imagebytes;q=0',
    'application/imagebytes;q=invalid',
  ]) {
...
it('yields while serializing and rejects an invalidated frame', async () => {
  ...
  const check = () => {
    if (!current) throw new Error('Discarded frame')
  }
...
it('negotiates both cameras and preserves a completed frame across binary and JSON reads', async () => {
  ...
  for (const number of [0, 1]) {
    ...
    const put = (
      member: string,
      payload: { Connected?: boolean; Duration?: number; Light?: boolean },
    ) => app.inject({ method: 'PUT', url: `${path}/${member}`, payload })
    ...
    const binary = await app.inject({
      url: `${path}/imagearray?ClientTransactionID=12`,
    ...
    for (const [x, y] of [
      ...
      expect(binary.rawPayload.readUInt16LE(44 + (x! * imageHeight + y!) * 2)).toBe(
        json.Value[x!][y!],
      )
    }
  }
...
it('returns negotiated ImageBytes error metadata when no frame is available', async () => {
  ...
  const response = await app.inject({
    url: '/api/v1/camera/1/imagearray?ClientTransactionID=19',
    headers: { accept: 'application/imagebytes' },
  })
