...
function Controls() {
  ...
  const exposing = state?.cameraActivity === 'exposing'
      ...
      {state && <div className="sim-layout">
        ...
        <div className="sim-side">
          <Panel title="Camera view" description="Try an exposure without visible stars.">
            <div className="sim-camera"><strong>{state.obscured ? 'Obscured' : 'Clear sky'}</strong><p>{state.obscured ? 'New exposures contain only background noise. A solve should fail.' : 'New exposures contain the generated star field.'}</p></div>
            <Button tone="neutral" disabled={blocked} onClick={() => void command('/simulator/camera', 'PUT', { obscured: !state.obscured }, 'Camera view updated for the next exposure')}>{state.obscured ? 'Clear the camera' : 'Obscure the camera'}</Button>
          </Panel>
