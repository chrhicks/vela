...
function Axis({
  ...
  return (
    <section aria-label={`${vertical ? 'Altitude' : 'Azimuth'} adjustment`}>
...
function Controls() {
  ...
  if (exposing) {
    ...
    statusMessage = 'Mount moving · wait before adjusting'
  } else if (pending) {
    statusMessage = 'Applying change…'
  } else if (notice === 'Connecting to simulator…') {
    statusMessage = 'Ready for adjustments'
  }

  function adjust(altitudeArcsec: number, azimuthArcsec: number) {
    return command(
      '/simulator/adjust',
      'PUT',
      { altitudeArcsec, azimuthArcsec },
      'Offsets applied · next exposure uses this position',
    )
  }

  function adjustAltitude(delta: number) {
    if (!state) return
    const altitude = Math.max(-18000, Math.min(18000, state.altitudeArcsec + delta))
    void adjust(altitude, state.azimuthArcsec)
  }

  function adjustAzimuth(delta: number) {
    if (!state) return
    const azimuth = Math.max(-18000, Math.min(18000, state.azimuthArcsec + delta))
    void adjust(state.altitudeArcsec, azimuth)
  }
    ...
    >
      <article className="sim-controls" data-pending={pending}>
        <header className="sim-heading">
          ...
          <span>{available ? 'Local simulator' : 'Disconnected'}</span>
        </header>
        ...
        {!available && (
          <p role="status" className="sim-notice">
            {state
              ? `Service unavailable. Showing last-known state from ${new Date(observedAt!).toLocaleTimeString()}. Reconnecting…`
              : 'Waiting for the local simulator service…'}
          </p>
        )}
        {state && (
          <div className="sim-layout">
            ...
            >
              ...
              <div className="sim-axes">
                <Axis
                  axis="altitude"
                  value={state.altitudeArcsec}
                  step={Number(step)}
                  disabled={blocked || mountBusy}
                  adjust={adjustAltitude}
                />
                <Axis
                  axis="azimuth"
                  value={state.azimuthArcsec}
                  step={Number(step)}
                  disabled={blocked || mountBusy}
              ...
              <div className="sim-details">
                ...
                >
                  {exactOpen ? '▾' : '▸'} Set exact offsets
                </button>
                {exactOpen && (
                  <form onSubmit={applyExact}>
                    <Input
                      name="altitude"
                      required
                      ...
                      max={18000}
                      step={1}
                      defaultValue={state.altitudeArcsec}
                    ...
                    <Input
                      ...
                      max={18000}
                      step={1}
                      defaultValue={state.azimuthArcsec}
                      message="Positive is east of north."
                    />
                    <Button type="submit" tone="neutral" disabled={blocked || mountBusy}>
                      Apply offsets
                    </Button>
                  </form>
                )}
            ...
            <div className="sim-side">
              ...
              >
                {state.cameras.map(camera => (
                  <section
                    ...
                    key={camera.number}
                    aria-label={camera.sensor === 'mono' ? 'Mono camera' : 'Color camera'}
                  >
                    <div className="sim-camera-title">
                    ...
                    <Select
                      ...
                      disabled={blocked || camera.activity === 'exposing'}
                      options={[
              ...
              >
                <div className="sim-camera">
                  ...
                  <p>
                    {state.obscured
                      ? 'New exposures contain background noise without stars.'
