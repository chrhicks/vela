...
const operations = [
  ...
  },
  {
    ...
    method: 'PUT',
    contentType: 'application/json',
    run: (client: AlpacaClient) =>
      client.command(camera, 'startexposure', { Duration: '1', Light: 'true' }),
  },
  {
...
it.each(operations)(
  'classifies a terminated $name body as transport without repeating the request',
  async operation => {
...
it('does not depend on the stream error being a TypeError', async () => {
  ...
  const failed = expect(client.connected(camera)).rejects.toMatchObject({
    reason: 'transport',
    cause,
  })
...
it.each([
  ...
  {
    name: 'wrong value schema',
    payload: JSON.stringify({ ...envelope, Value: 'true' }),
    cause: Error,
  },
  ...
  await expect(client.readBoolean(camera, 'imageready')).rejects.toMatchObject({
    reason: 'invalid-response',
    cause: expect.any(cause),
  })
})
...
it('does not mistake a decoder TypeError for a stream failure', async () => {
  ...
  const schema = Schema.Boolean.check(
    Schema.makeFilter(() => {
      throw cause
    }),
  )
  ...
  const client = createAlpacaClient({
    baseUrl: 'http://fake',
    fetch: async () => Response.json({ ...envelope, Value: true }),
  })

  await expect(client.readValue(camera, 'imageready', schema)).rejects.toMatchObject({
    reason: 'invalid-response',
    cause,
  })
})
...
it.each([
  ...
  {
    contentType: 'application/json',
    payload: JSON.stringify({ ...envelope, Type: 2, Rank: 3, Value: [[1]] }),
  },
  { contentType: 'application/imagebytes', payload: new Uint8Array(43) },
])(
  'keeps malformed completed image data invalid ($contentType)',
...
it.each(['application/json', 'application/imagebytes'])(
  'preserves caller cancellation during a %s image body',
  async contentType => {
    ...
    const fetch: typeof globalThis.fetch = async (_input, init) => {
      init!.signal!.addEventListener('abort', () => body.controller.error(init!.signal!.reason), {
        once: true,
      })
...
it('times out an ordinary read while its body is pending and preserves the abort cause', async () => {
  ...
  const fetch = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    requestSignal.addEventListener('abort', () => body.controller.error(requestSignal.reason), {
      once: true,
    })
