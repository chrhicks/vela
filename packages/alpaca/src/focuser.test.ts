...
function observatory(requestTimeoutMs = 100) {
  ...
  const fetch: typeof globalThis.fetch = async (input, init) => {
    ...
    const envelope = (Value?: ResponseFixture, ErrorNumber = 0, ErrorMessage = '') =>
      Response.json({
...
describe('focuser write boundary', () => {
  ...
  it('refuses a target outside the walk window before writing', async () => {
    const fake = observatory()
    await expect(
      fake.focuser.move({ focuserId: 'eaf-id', position: 30000, window }),
    ).rejects.toThrow(/travel window around the starting position/)
    expect(fake.writes).toEqual([])
  })
  ...
  it('treats a lost move response as success only when inspection shows the target, and never replays', async () => {
    ...
    expect(await arrived.focuser.move({ focuserId: 'eaf-id', position: 33042, window })).toEqual({
      position: 33042,
    })
    ...
    missed.state.onMove = () => {
      missed.values.position = 32842
    }

    await expect(
      missed.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toThrow(/did not confirm the commanded position/)
  ...
  it('skips a write when the focuser is already at the target', async () => {
    const fake = observatory()
    expect(await fake.focuser.move({ focuserId: 'eaf-id', position: 32842, window })).toEqual({
      position: 32842,
    })
    expect(fake.writes).toEqual([])
  })
  ...
  it('does not write move when the focuser is disconnected, relative, or already moving', async () => {
    ...
    await expect(
      disconnected.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ...
    await expect(
      relative.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ...
    await expect(
      busy.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toThrow(/already moving/)
  ...
  it('does not write move when position, MaxStep, or IsMoving cannot be read as integers and booleans', async () => {
    ...
    await expect(
      fractional.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ...
    await expect(
      overRange.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toMatchObject({
      name: 'AlpacaProviderError',
      reason: 'invalid-response',
      endpoint: 'position',
    ...
    await expect(
      zeroMax.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toMatchObject({
      name: 'AlpacaProviderError',
      reason: 'invalid-response',
      endpoint: 'position',
    ...
    await expect(
      fractionalMax.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toMatchObject({
      name: 'AlpacaProviderError',
      reason: 'invalid-response',
      endpoint: 'position',
    ...
    await expect(
      movingFlag.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toMatchObject({
      name: 'AlpacaProviderError',
      reason: 'invalid-response',
    })
  ...
  it('keeps a rejected move’s driver reason and does not replay', async () => {
    ...
    await expect(
      fake.focuser.move({ focuserId: 'eaf-id', position: 33042, window }),
    ).rejects.toThrow(/Temperature compensation is enabled.*did not repeat the move/)
    expect(fake.writes.map(write => write.operation)).toEqual(['move', 'halt'])
