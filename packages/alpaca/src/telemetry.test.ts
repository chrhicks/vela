...
it('keeps tracing optional when no SDK provider is registered', async () => {
  const client = createAlpacaClient({
    baseUrl: 'http://fake',
    fetch: async () => Response.json(envelope(12)),
  })
...
it('keeps the ALPACA span open through headers, body and value validation', async () => {
  ...
  expect(span.attributes).toMatchObject({
    ...
    'url.path': '/api/v1/telescope/0/rightascension',
    'server.address': 'fake',
    'url.scheme': 'http',
    'http.response.status_code': 200,
    'alpaca.operation': 'rightascension',
    'alpaca.device.type': 'telescope',
    'alpaca.device.number': 0,
    'alpaca.client_transaction_id': 7,
    'alpaca.server_transaction_id': 23,
    'alpaca.response.value': 12,
    'alpaca.error_number': 0,
  })
  ...
  expect(span.events.map(event => event.name)).toEqual(
    expect.arrayContaining([
      'alpaca.request.dispatched',
      'alpaca.response.headers',
      'alpaca.response.body',
      'alpaca.response.decode',
    ]),
  )
})
...
it('marks HTTP 200 protocol errors and malformed values as errors, including binary image envelopes', async () => {
  ...
  const command = createAlpacaClient({
    baseUrl: 'http://fake',
    fetch: async () => Response.json(envelope(undefined, 1025)),
  })

  await expect(
    command.command(telescope, 'moveaxis', { Axis: '0', Rate: '0' }),
  ).rejects.toMatchObject({ errorNumber: 1025 })

  const malformed = createAlpacaClient({
    baseUrl: 'http://fake',
    fetch: async () => Response.json(envelope('not a number')),
  })

  await expect(malformed.readNumber(telescope, 'rightascension')).rejects.toMatchObject({
  ...
  const image = createAlpacaClient({
    baseUrl: 'http://fake',
    fetch: async () =>
      new Response(bytes, { headers: { 'content-type': 'application/imagebytes' } }),
  })
...
it.each([false, true])(
  'correlates rotation requests and independent confirmed cleanup (cancel=%s)',
  async cancel => {
    const exporter = recording()
    let moving = false
    let reading!: () => void
    ...
    if (cancel) {
      ...
      await pendingRead
      expect(
        exporter.getFinishedSpans().some(span => span.attributes['alpaca.command.rate'] === '3'),
      ).toBe(true)
      expect(
        exporter.getFinishedSpans().some(span => span.name === 'alpaca.rotate_right_ascension'),
      ).toBe(false)
    ...
    expect(
      requests.every(span => span.spanContext().traceId === parent.spanContext().traceId),
    ).toBe(true)
    expect(
      requests.every(span => span.parentSpanContext?.spanId === rotation.spanContext().spanId),
    ).toBe(true)
    const stop = requests.find(span => span.attributes['alpaca.command.rate'] === '0')!
    expect(stop.status.code).toBe(SpanStatusCode.OK)
    expect(rotation.events.map(event => event.name)).toEqual(
      expect.arrayContaining([
        'alpaca.stop.requested',
        'alpaca.stop.acknowledged',
        'alpaca.stop.confirmed',
      ]),
    )
