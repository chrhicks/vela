    ...
    <div className="vela-polar-baseline">
      <Panel className="vela-polar-baseline__summary">
        ...
        {step ? (
          ...
          <dl className="vela-polar-setup-facts"><div><dt>Camera</dt><dd>{physical ? 'ASI2600MC Pro' : 'Main imaging camera'}</dd></div><div><dt>Exposure</dt><dd>30 seconds</dd></div><div><dt>Starting point</dt><dd>{physical ? '10° from home · Dec +80°' : 'Current position'}</dd></div></dl>
        )}
      </Panel>
      <div className="vela-polar-baseline__next">
        <h3>{step ? 'What happens next' : 'Before you start'}</h3>
        <p>{step ? 'After the third solve, the adjustment view will show your alignment error and the target reticle.' : physical ? 'Each attempt homes, then moves 10° away from the pole before taking images. Prepare a clear movement corridor: Vela checks RA direction within 1° on either side, then rotates RA westward in two roughly 18° steps. Allow up to 40° total westward movement.' : 'Point toward a clear patch of sky. Starting measurement will move the telescope through three positions along its rotation axis.'}</p>
