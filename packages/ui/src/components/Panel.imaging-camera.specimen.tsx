...
function ImagingCameraPreview({ props, onPropsChange }: { props: Props, onPropsChange?: (patch: Props) => void }) {
  ...
  return <article className="vela-imaging-demo">
    ...
    <main>
      <header className="vela-imaging-heading"><div><p>Askar FRA 400</p><h1>Observe</h1></div><Badge tone={state === 'offline' ? 'warning' : 'positive'}>{state === 'offline' ? 'Updates interrupted' : 'Connected'}</Badge></header>
      <p className="vela-imaging-intro">{state === 'offline' ? 'Showing the saved setup while rig updates are interrupted.' : 'Your rig, ready for the next adjustment.'}</p>
      <Panel className="vela-imaging-setup">
        <div className="vela-imaging-summary">
          <div><h2>Imaging camera</h2><p>{saved === 'none' ? 'Choose the camera Capture will use.' : name}</p>{saved !== 'none' && <small>{slot} · Saved for this rig</small>}</div>
      ...
      <div className="vela-imaging-activities">
        <Panel title="Capture"><p>Take an exposure and inspect what the camera sees.</p><span>{saved === 'none' ? 'Choose an imaging camera above to begin.' : unavailable ? 'Camera needs attention before capture.' : state === 'busy' ? 'Exposing · 12 / 30 s' : `${name} · Ready`}</span><Button tone="accent" disabled={saved === 'none' || unavailable || editing} onClick={() => update({ state: state === 'busy' ? 'ready' : 'busy' })}>{state === 'busy' ? 'Finish sample exposure' : 'Try capture state'} →</Button></Panel>
        <Panel title="Polar alignment"><p>Measure your alignment and adjust the mount when you need to.</p><span>Available independently of imaging-camera setup.</span></Panel>
      </div>
    </main>
...
export const specimen: ComponentSpecimen = {
  componentId: 'panel', componentName: 'Panel / Card', id: 'panel-imaging-camera',
