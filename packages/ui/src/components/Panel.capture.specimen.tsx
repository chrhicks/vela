...
function CapturePreview({ props, onPropsChange }: {
  ...
  const activity = phase === 'exposing' ? 'Exposing' : phase === 'reading' ? 'Receiving image' : disconnected ? 'Connection interrupted' : phase === 'failed' ? 'Exposure failed' : phase === 'stopped' ? 'Exposure stopped' : phase === 'complete' ? 'Image received' : 'Ready for an exposure'
  ...
  const rigContext = <details className="vela-capture-rig">
    ...
    <dl><div><dt>Simulator Camera</dt><dd>{disconnected ? 'Last known: connected' : busy ? activity : 'Connected · idle'}</dd></div><div><dt>Simulator Telescope</dt><dd>{disconnected ? 'Last known: tracking' : 'Connected · tracking'}</dd></div></dl>
  </details>

  return <article className="vela-capture-demo">
    ...
    <main className="vela-capture-main">
      ...
      {screen === 'observe' ? <>
        <p className="vela-capture-intro">{disconnected ? `Waiting for rig updates.${hasImage ? ' Your last image is still available.' : ''}` : 'Your rig is connected. What would you like to do?'}</p>
        {rigContext}
        ...
        {disconnected || phase === 'failed' ? <div className="vela-capture-warning" role="alert">
          ...
          <p>{disconnected ? 'Capture status is unknown. Waiting for the rig to reconnect.' : 'The camera did not return a new image.'}{hasImage ? ' The last image is kept below.' : ''}</p>
        </div> : null}
        <div className="vela-capture-layout">
          <section className="vela-capture-image" aria-label="Latest image">
            ...
            <div className="vela-capture-image__window" data-zoomed={hasImage && zoomed || undefined} ref={imageWindow} tabIndex={hasImage && zoomed ? 0 : undefined} role={hasImage && zoomed ? 'region' : undefined} aria-label={hasImage && zoomed ? 'Image at 100 percent. Scroll to inspect.' : undefined}>
              {hasImage ? <img src={sampleImage} width="1600" height="1200" alt="Monochrome simulator exposure showing a dense star field" /> : <div className="vela-capture-empty"><CameraMark /><h3>{busy ? 'Taking your first exposure' : 'Your first image starts here'}</h3><p>{busy ? 'You can watch the progress beside this view.' : 'Choose an exposure time, then take an image to check what the camera sees.'}</p></div>}
            </div>
            {hasImage && <footer><span>{imageSeconds} s <i>·</i> Mono <i>·</i> 1600 × 1200</span><span>{zoomed ? 'Scroll to inspect' : 'Display stretched'}</span></footer>}
          </section>
          ...
          <Panel className="vela-capture-controls" title="Take an exposure">
            ...
            <div className="vela-capture-command">
              {phase === 'exposing' ? <Button size="large" onClick={() => { setPlaying(false); update({ phase: 'stopped' }) }}>Stop exposure</Button>
                : <Button size="large" tone="accent" disabled={phase === 'reading' || disconnected || !validExposure} onClick={takeExposure}>{phase === 'reading' ? 'Receiving image…' : 'Take exposure'}</Button>}
            </div>
            <div className="vela-capture-progress">
              ...
              <p>{busy ? hasImage ? 'The previous image stays visible until the new one arrives.' : 'The image will appear when the exposure is received.' : phase === 'stopped' ? 'No new image was added.' : 'One exposure at a time. The latest image stays here.'}</p>
            </div>
          </Panel>
