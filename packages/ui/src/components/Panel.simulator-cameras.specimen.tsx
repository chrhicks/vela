...
function CamerasPreview({ props, onPropsChange }: {
  ...
  return (
    <article className="vela-sim-cameras">
      <header>
        <div>
          <p>Development rig</p>
          <h1>Rig simulator</h1>
      ...
      <div className="vela-sim-cameras-grid">
        <Panel title="Cameras" description="Choose each camera’s image size for the next exposure.">
          {cameras.map(camera => (
            <section
              className="vela-sim-camera-row"
              key={camera.number}
              aria-label={camera.sensor === 'mono' ? 'Mono camera' : 'Color camera'}
            >
              <div className="vela-sim-camera-title">
              ...
              <Select
                ...
                disabled={!available || camera.activity === 'exposing'}
                options={[
        ...
        <div className="vela-sim-cameras-side">
          <Panel title="Shared sky" description="Both cameras see the same sky and mount position.">
            ...
            <p className="vela-sim-cameras-copy">
              {obscured
            ...
            >
              {obscured ? 'Clear the sky' : 'Obscure the sky'}
            </Button>
