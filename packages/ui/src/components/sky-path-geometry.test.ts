...
describe('overhead sky geometry', () => {
  it('puts east on the left and never folds below-horizon altitude into the dome', () => {
    ...
    ).toBe('M100.00,20.00 L100.00,37.78')
  })
...
it('measures angular separation on the sphere rather than the flat dome', () => {
  expect(
    angularSeparationDegrees(
      ...
      { azimuthDegrees: 90, altitudeDegrees: 0 },
    ),
  ).toBeCloseTo(90)
  expect(
    angularSeparationDegrees(
      { azimuthDegrees: 0, altitudeDegrees: 60 },
      { azimuthDegrees: 180, altitudeDegrees: 60 },
    ),
  ).toBeCloseTo(60)
  expect(
    angularSeparationDegrees(
      { azimuthDegrees: 359, altitudeDegrees: 0 },
      { azimuthDegrees: 1, altitudeDegrees: 0 },
    ),
  ).toBeCloseTo(2)
})
