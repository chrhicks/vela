...
function LightPreview({
  ...
  const samples = getSkySamples(target, 12, 97, 15).map((sample, index) => ({
    ...sample,
    light: phaseAt(index),
  }))
  ...
  return (
    <div className="vela-sky-path-specimen">
      <p className="vela-sky-path-specimen__caption">
        Sample day and night · invented light windows
      </p>
      <SkyPath
        samples={samples}
...
export const specimen: ComponentSpecimen = {
  ...
  name: 'Light windows',
  description:
    'Colored target-path segments distinguish daylight, twilight stages and astronomical darkness. Invented 24-hour fixtures; no production solar data.',
  controls: {
  ...
  render: (props, onPropsChange) => (
    <LightPreview props={props} {...(onPropsChange ? { onPropsChange } : {})} />
  ),
}
