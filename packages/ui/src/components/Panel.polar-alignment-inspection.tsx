...
export const alignmentInspectionFixtures = {
  'large-error': { x: 580, y: 478, total: '8′ 23″', azimuth: '7′ 20″', altitude: '4′ 04″' },
  'near-aligned': { x: 794.5, y: 595.5, total: '14″', azimuth: '11″', altitude: '9″' },
  'outside-image': { x: -220, y: 356, total: '34′ 58″', azimuth: '34′', altitude: '8′ 08″' },
}
...
function ImageCanvas({ target, view }: { readonly target: Target | undefined; readonly view: ImageView }) {
  ...
  const height = full ? frame.height : view === 'fine' ? 30 : Math.max(120,
  ...
  return <div className="vela-polar-image-canvas"><svg viewBox={`${left} ${top} ${width} ${height}`} role="img" aria-label={target ? 'Illustrative exposure with frame reference and correction target' : 'Full baseline exposure, no alignment solution'}>
    <image href={starField} width={frame.width} height={frame.height} />
    {target && <>
      <path d={`M${frame.x} ${frame.y}L${target.x} ${target.y}`} fill="none" stroke="var(--vela-polar-reference)" strokeWidth={1.5 * scale} />
      <circle data-marker="reference" cx={frame.x} cy={frame.y} r={9 * scale} fill="none" stroke="var(--vela-polar-reference)" strokeWidth={1.5 * scale} />
      <g data-marker="target" fill="none" stroke="var(--vela-polar-target)" strokeWidth={1.4 * scale}>
        ...
        <path d={`M${target.x - 30 * scale} ${target.y}h${20 * scale}m${20 * scale} 0h${20 * scale}M${target.x} ${target.y - 30 * scale}v${20 * scale}m0 ${20 * scale}v${20 * scale}`} />
      </g>
    </>}
...
function InspectionView({ target, status }: { readonly target: Target | undefined; readonly status: string }) {
  ...
  return <div className="vela-polar-inspection">
    ...
    {target && view !== 'native' && <div className="vela-polar-inspection-legend"><span><i /> Frame reference</span><span><i /> Correction target</span></div>}
  </div>
}
...
export function AlignmentImageInspection({ target, title, status }: {
  ...
  return <figure className="vela-polar-image">
    <div className="vela-polar-image-heading"><span>{title}</span><Button size="small" onClick={() => setExpanded(true)}>Enlarge image</Button></div>
    <InspectionView target={target} status={status} />
    <figcaption>Exposure started <time dateTime="2026-09-14T20:25:49">8:25:49 PM</time> · Fixture</figcaption>
    <Dialog open={expanded} title={title} description="Same exposure · Started 8:25:49 PM · Inspection takes no new image." onDismiss={() => setExpanded(false)} className="vela-polar-inspection-dialog">
      <InspectionView target={target} status={status} />
    </Dialog>
  </figure>
}
