...
import { horizonAt, horizonSectors, projectSky, visibleSkyPath, type HorizonPoint, type SkyCoordinate } from './sky-path-geometry'
...
export function SkyPath({ samples, targetName, selectedIndex, onSelectedIndexChange, nowIndex, horizon, marginDegrees = 5, onMarginDegreesChange, compact = false }: SkyPathProps) {
  ...
  const labels = skyLabels(samples, now, selected, size, compact)
  ...
  return (
    <div ref={root} className="vela-sky-path" data-compact={compact || undefined}>
      ...
      <svg className="vela-sky-path__map" viewBox={`0 0 ${size} ${size}`} role="img" aria-labelledby={`${id}-title ${id}-description`}>
        ...
        <desc id={`${id}-description`}>Overhead sky dome. North is up, east is left. Outer ring is altitude zero; center is altitude 90 degrees. {selected ? `${selected.label}, altitude ${selected.altitudeDegrees.toFixed(0)} degrees. ${selectedStatus}.` : selectedStatus}</desc>
        <defs>
      ...
      <input id={`${id}-time`} className="vela-sky-path__range" type="range" min="0" max={Math.max(1, samples.length - 1)} step="1" value={Math.max(0, index)} disabled={samples.length < 2} aria-valuetext={selected ? `${selected.label}, ${selected.altitudeDegrees.toFixed(0)} degrees altitude. ${selectedStatus}` : 'No path samples'} onChange={event => onSelectedIndexChange(Number(event.target.value))} />
...
function skyLabels(samples: readonly SkyPathSample[], now: SkyPathSample | undefined, selected: SkyPathSample | undefined, size: number, compact: boolean) {
  const center = size / 2
  const radius = center - 27
  ...
  for (const sample of [now, selected]) {
  ...
  return { now: nowLabel, altitudes, times }
}
