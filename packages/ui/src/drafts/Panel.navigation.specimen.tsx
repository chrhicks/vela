...
function NavigationPreview({ props, onPropsChange }: { props: Props; onPropsChange?: (patch: Props) => void }) {
    ...
    switch (page) {
      ...
      case 'capture':
        return currentCapture
          ? interrupted
            ? 'Connection interrupted · last known capture shown.'
            : 'The latest exposure stays close at hand.'
          : 'No capture is running on this rig.'
      case 'observe':
  ...
  function renderPage() {
    switch (page) {
      case 'targets':
        return (
          <div className="vela-nav-demo__targets">
        ...
        return (
          <div className="vela-nav-demo__capture">
            ...
            <Panel title="Latest exposure">
              <p>
                {completed > 0
                  ? `Frame #${completed} · 60 seconds · synthetic image`
          ...
          <div className="vela-nav-demo__observe">
            ...
            >
              <p>
                {currentCapture
                  ? interrupted
                    ? `Last seen exposing #${completed + 1}. Open Capture to inspect what is known.`
  ...
  const captureStatus = interrupted
    ? `Last seen exposing #${completed + 1}`
    : reading
      ? `Reading exposure #${completed + 1}`
      : `Exposing #${completed + 1} · ${60 - elapsed} seconds remaining`
  ...
  return (
    <section className="vela-nav-demo" aria-label="Navigation experiment">
          ...
          {rig === 'all' ? (
            <>
              <div className="vela-nav-demo__heading">
                <p>Your observatory</p>
                <h1>Choose a rig</h1>
                <span>Opening a rig changes the view. Its work keeps running.</span>
              </div>
              <div className="vela-nav-demo__rig-cards">
                {Object.entries(rigs).map(([id, name]) => (
                  <Panel key={id}>
                    <h2>{name}</h2>
                    <p>
                      {id === 'askar' && activity
      ...
      <footer className="vela-nav-demo__try">
        <label>
          Try the bar with
          <select value={phase} onChange={event => update({ phase: event.target.value })}>
            <option value="idle">Nothing running</option>
            <option value="capturing">Askar capturing</option>
            <option value="reading">Askar reading an image</option>
            <option value="interrupted">Askar updates interrupted</option>
          </select>
        </label>
        {phase === 'capturing' && (
          <label>
            ...
            <input
              ...
              max="60"
              value={elapsed}
              onChange={event => update({ elapsed: Number(event.target.value) })}
            />
            <span>{elapsed}s</span>
          </label>
        )}
