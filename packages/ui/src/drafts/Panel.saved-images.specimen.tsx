    ...
    <main className="vela-capture-main">
      ...
      {screen === 'observe' && <>
        ...
        <div className="vela-capture-hub">
          <Panel className="vela-capture-entry"><div className="vela-capture-entry__preview">{hasImage && image(frame)}<span>{hasImage ? 'Latest exposure' : 'No exposure yet'}</span></div><div className="vela-capture-entry__body"><h2>Capture</h2><p>Take exposures and inspect the latest image.</p><div className="vela-capture-entry__status">{busy ? 'Capturing' : 'Ready for an exposure'}</div><Button tone="accent" onClick={() => update({ screen: 'capture' })}>Open capture →</Button></div></Panel>
          <Panel className="vela-capture-entry"><div className="vela-capture-entry__body"><h2>Saved images</h2><p>Your retained frames, ready to browse and take into your processing tools.</p><div className="vela-capture-entry__status">{saved.length} images · Original FITS + preview</div><Button onClick={() => update({ screen: 'saved' })}>Browse saved images →</Button></div></Panel>
        </div>
      </>}
      {screen === 'capture' && <>
        {state === 'save-error' && <div className="vela-capture-warning" role="alert"><strong>Image could not be saved</strong><p>Capture is stopped. This image is still available below. Check storage, then try keeping it again.</p></div>}
        <div className="vela-capture-layout">
          <section className="vela-capture-image" aria-label="Latest image"><header><div><h2>Latest image</h2><span>{hasImage ? `${time(frame)}${busy ? ' · Previous exposure' : ''}` : 'No exposure yet'}</span></div>{hasImage && <div className="vela-saved-image-actions">{scaleControls}{isSaved ? <Badge tone="positive">Saved</Badge> : <Button size="small" onClick={keepImage}>Keep this image</Button>}</div>}</header>
            <div className="vela-capture-image__window" ref={imageWindow} data-zoomed={hasImage && zoomed || undefined} tabIndex={hasImage && zoomed ? 0 : undefined} role={hasImage && zoomed ? 'region' : undefined} aria-label={hasImage && zoomed ? 'Image at 100 percent. Scroll to inspect.' : undefined}>{hasImage ? image(frame) : <div className="vela-capture-empty"><h3>Your first image starts here</h3><p>Take an exposure to inspect it here.</p></div>}</div>
            {hasImage && <><div className="vela-capture-image-statistics"><dl><div><dt>Dimensions</dt><dd>1600 × 1200</dd></div><div><dt>Stars</dt><dd>146</dd></div><div><dt>HFR · px</dt><dd>2.24</dd></div></dl></div><footer><span>{String(values.imageSeconds)} s · Color · Display stretched</span><span>{isSaved ? 'FITS + preview saved' : 'Not saved'}</span></footer></>}
          </section>
          <Panel className="vela-capture-controls" title="Capture images">
            <div className="vela-capture-camera"><div><strong>Simulator Color Camera</strong><span>Imaging camera</span></div></div>
            <Input label="Exposure · seconds" type="number" min="0.1" max="600" step="0.1" disabled={busy} value={String(values.exposure)} invalid={!validExposure} onChange={event => update({ exposure: event.target.value })} />
            <Checkbox label="Repeat until stopped" checked={Boolean(values.repeat)} disabled={busy} onChange={event => update({ repeat: event.target.checked })} />
            <Checkbox label="Save frames" description="Keep every completed image as FITS + preview." checked={Boolean(values.saveFrames)} disabled={busy} onChange={event => update({ saveFrames: event.target.checked })} />
            <div className="vela-capture-command"><Button size="large" tone={busy ? 'neutral' : 'accent'} disabled={!busy && !validExposure} onClick={() => { setPlaying(!busy); update({ state: busy ? 'idle' : 'capturing' }) }}>{busy ? 'Stop capture' : values.repeat ? 'Start run' : 'Take exposure'}</Button></div>
            <div className="vela-capture-progress"><div><strong role="status">{busy ? 'Exposing' : state === 'save-error' ? 'Capture stopped' : 'Ready for an exposure'}</strong></div><p>{values.saveFrames ? 'Completed frames go to Saved images for this rig.' : 'Frames are disposable. Use Keep this image to save one you want.'}</p></div>
          </Panel>
        </div>
      </>}
      {screen === 'saved' && <>
...
export const specimen: ComponentSpecimen = {
  componentId: 'panel', componentName: 'Panel / Card', id: 'panel-saved-images', name: 'Saved images · Draft product example',
  ...
  defaultProps: { screen: 'capture', state: 'idle', hasImage: true, saveFrames: false, repeat: true, saveFailure: false, saved: '4,3,2,1', exposure: '10', imageSeconds: '10', exposures: '', frame: 5, selected: 4 },
