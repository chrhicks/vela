...
function CompactSkyPath({ targetId, nowIndex }: { targetId: string; nowIndex: number }) {
  ...
  return (
    <svg
      className="vela-target-path"
      viewBox="0 0 320 140"
      role="img"
      aria-label="Sample altitude through the night. Local obstructions not included."
    >
...
function TargetFramingPreview({
  ...
  if (phase === 'exposing') {
    status = 'Taking test exposure · preview simulation'
  } else if (phase === 'settling') {
    status = 'Waiting for the mount to settle'
  } else if (busy) {
    status = 'Slewing · preview simulation'
  } else if (checking) {
    status = 'Framing check · example offset'
  } else if (phase === 'failed') {
    status = 'Slew not confirmed'
  } else if (phase === 'stopped') {
    status = 'Preview stopped'
  } else {
    status = 'Ready to frame'
  }

  return (
    <div className="vela-target-example" data-expanded={skyExpanded} ref={example}>
      <article className="vela-target-demo" inert={skyExpanded}>
        <header className="vela-target-shell">
          <strong>Vela</strong>
        ...
        <main className="vela-target-main">
          {composing && (
            <Button
              ...
              tone="quiet"
              disabled={busy}
              onClick={() => update({ screen: 'browse', phase: 'idle' })}
            >
              ← Targets
            </Button>
          )}
                ...
                {matches.map(item => (
                  ...
                  >
                    ...
                    <div className="vela-target-card-copy">
                      ...
                      <CompactSkyPath targetId={item.id} nowIndex={nowIndex} />
            ...
            <div className="vela-target-layout">
              <section className="vela-target-composition" aria-label="Composition">
                ...
                >
                  ...
                  {hasCheck && (
                    <div
                      className="vela-target-frame vela-target-frame--actual"
                      style={{ left: `${x + 5}%`, top: `${y - 3}%` }}
                    />
                  )}
                </div>
                <footer>
                ...
                <div className="vela-target-adjustments">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      value={x}
                      disabled={busy || checking}
                      onChange={event => update({ frameX: Number(event.target.value) })}
                    />
                  </label>
                  <label>
                    Vertical position
                    <input
                      ...
                      value={y}
                      disabled={busy || checking}
                      onChange={event => update({ frameY: Number(event.target.value) })}
                    />
                  </label>
                </div>
              </section>
              <aside className="vela-target-sidebar">
                ...
                <Panel title="Your composition">
                  ...
                  <dl className="vela-target-details">
                    ...
                    <div>
                      <dt>Orientation</dt>
                      <dd>Fixed · example frame</dd>
                    </div>
                    <div>
                      <dt>Position</dt>
                      <dd>{x === 50 && y === 50 ? 'Reference center' : 'Custom composition'}</dd>
                    </div>
                  </dl>
                  <div className="vela-target-command">
                    ...
                    {checking ? (
                      ...
                      <>
                        ...
                        >
                          {phase === 'failed' ? 'Restart preview' : 'Preview slew & check'}
                        </Button>
                      </>
                    )}
                    {!busy && (
        ...
        <section className="vela-target-credits" aria-label="Reference image credits">
      ...
      <Dialog
        open={skyExpanded}
        onDismiss={dismissSky}
        title={`${target.name} · Through the night`}
        description="Sample night · imaginary observing site"
