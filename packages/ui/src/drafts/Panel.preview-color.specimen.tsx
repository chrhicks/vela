    ...
    <>
      {native && (
        <div className="vela-preview-color__native-tools">
          ...
          <Button size="small" onClick={center}>Center image</Button>
        </div>
      )}
      ...
      <div className="vela-preview-color__pair" data-scale={scale}>
        {['current', 'neutral'].map((treatment, index) => (
          <figure key={treatment}>
            <figcaption>
              <strong>{treatment === 'current' ? 'A · Current preview' : 'B · Bounded background'}</strong>
              <span>
                {treatment === 'current'
  ...
  return (
    <article className="vela-preview-color">
      <header>
        ...
        <h1>Preview color</h1>
        <p>Judge the sky background, star profiles and faint structure. Both views are display aids, not calibrated color.</p>
      </header>
      <div className="vela-preview-color__controls">
        ...
        <div className="vela-preview-color__scale" aria-label="Preview scale">
          {Object.entries({ fit: 'Fit', native: '100%', thumbnail: 'Thumbnail' }).map(([value, label]) => (
            <Button
              key={value}
              size="small"
      ...
      <details>
        ...
        <dl>
          ...
          <dt>Dimensions</dt>
          <dd>{fixture.width} × {fixture.height}; top-down sensor orientation</dd>
          <dt>Renderer</dt>
          <dd>{corpus.renderer} · baseline {corpus.baseline}</dd>
          <dt>Linked range</dt>
          <dd>{fixture.range.black}–{fixture.range.ceiling} ADU · asinh strength 10 in both views</dd>
        </dl>
