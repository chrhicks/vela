...
const suggestions = [
  ...
  { ...targets[0]!, family: 'Clusters', window: '1h 15m remaining', altitude: '39° now · setting', reason: 'A shorter opportunity: start soon to catch it higher.', filter: 'Broadband is the better fit', detail: 'A cluster’s stars emit across the visible spectrum.' },
]
...
function Discovery({ props, onPropsChange }: { props: Props; onPropsChange?: (patch: Props) => void }) {
  ...
  return <section className="vela-discovery-demo" aria-label="Target discovery">
    ...
    <div className="vela-discovery-demo__grid" aria-busy={refreshing}>{shown.map((target, offset) => <article className="vela-discovery-demo__card" key={target.id}>
      <div className="vela-discovery-demo__image"><img src={target.image} alt={`${target.name} reference photograph`} /><span>{target.kind}</span></div>
      <div className="vela-discovery-demo__body"><p className="vela-discovery-demo__catalog"><span className="vela-discovery-demo__rank">{page * 2 + offset + 1}</span>{target.catalog}</p><h2>{target.name}</h2><div className="vela-discovery-demo__window"><strong>{target.window}</strong><span>{target.altitude}</span></div><p className="vela-discovery-demo__reason">{target.reason}</p><div className="vela-discovery-demo__advice"><strong>{target.filter}</strong><p>{target.detail}</p></div><Button tone="quiet" onClick={() => update({ selected: target.name })}>Explore target <span aria-hidden="true">→</span></Button><small>Reference image · <a href={target.source} target="_blank" rel="noreferrer">{target.credit}</a></small></div>
    </article>)}</div>
    ...
    <footer className="vela-discovery-demo__footer"><span>{matches.length ? page * 2 + 1 : 0}–{Math.min(page * 2 + 2, matches.length)} of {matches.length} suggestions</span><div><Button size="small" tone="quiet" disabled={page === 0} onClick={() => update({ page: page - 1 })}>Previous</Button><Button size="small" tone="quiet" disabled={(page + 1) * 2 >= matches.length} onClick={() => update({ page: page + 1 })}>Next</Button></div></footer>
...
export const specimen: ComponentSpecimen = {
  componentId: 'panel', componentName: 'Panel / Card', id: 'panel-target-discovery', name: 'Target discovery · Product example',
  ...
  controls: {
    filter: { type: 'select', label: 'Object type', options: ['All objects', 'Emission', 'Reflection & dark', 'Galaxies', 'Clusters', 'Planetary'] },
    query: { type: 'text', label: 'Search targets' },
    light: { type: 'select', label: 'Light preference', options: ['All light', 'L-Ultimate subjects', 'Broadband subjects'] },
  ...
  defaultProps: { filter: 'All objects', query: '', light: 'All light', state: 'cached', page: 0, selected: '' },
